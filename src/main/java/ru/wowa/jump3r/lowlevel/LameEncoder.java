...
import ru.wowa.jump3r.mp3.MPEGMode;
...
import ru.wowa.jump3r.mp3.VbrMode;
...
public class LameEncoder /* implements org.jma.encoder.audio.IAudioEncoder */ {
	...
	// suggested maximum buffer BAG_SIZE for an mpeg frame
	private static final int DEFAULT_PCM_BUFFER_SIZE = 2048 * 16;

	// frame BAG_SIZE=576 for MPEG2 and MPEG2.5
	// =576*2 for MPEG1

	private final static int DEFAULT_QUALITY = QUALITY_MIDDLE;
	private final static int DEFAULT_BITRATE = BITRATE_AUTO;
	private final static int DEFAULT_CHANNEL_MODE = CHANNEL_MODE_AUTO;
	// in VBR mode, bitrate is ignored.
	private final static boolean DEFAULT_VBR = false;
  ...
  public LameEncoder(AudioFormat sourceFormat, AudioFormat targetFormat) {
		readParams(targetFormat.properties());
  ...
  public LameEncoder(AudioFormat sourceFormat, int bitRate, int channelMode, int quality, boolean VBR) {
	...
	private void readParams(@Nullable Map<String, Object> props) {
		if (props != null) {
			readProps(props);
		}
	}
	...
	public void setFormat(@Nullable AudioFormat sourceFormat, @Nullable AudioFormat targetFormat) {
    ...
    lame = new Lame(ga, bs, presets, qupvt, qu, vbr, ver, id3, mpg);
		...
		if (mode != CHANNEL_MODE_AUTO) {
			gfp.mode = Enum.valueOf(MPEGMode.class, chmode2string(mode).toUpperCase());
		}
		if (VBR) {
	...
	/**
	 * Returns the buffer needed pcm buffer BAG_SIZE. The passed parameter is a
	 * wished buffer BAG_SIZE. The implementation of the encoder may return a lower
	 * or higher buffer BAG_SIZE. The encoder must be initalized (i.e. not closed)
...
//					System.out.println(Arrays.toString(sample));
		...
		if (gfp.num_channels == 2) {
			for (int i = samples_read; --i >= 0;) {
				buffer[1][i] = sample_buffer[--p1];
				buffer[0][i] = sample_buffer[--p1];
			}
		} else if (gfp.num_channels == 1) {
			Arrays.fill(buffer[1], 0, samples_read, 0);
			for (int i = samples_read; --i >= 0;) {
				buffer[0][i] = buffer[1][i] = sample_buffer[--p1];
			}
		}
	...
	private void readProps(Map<String, Object> props) {
		...
		if (q instanceof String) {
			...
			vbrMode = ((Boolean) q).booleanValue();
		} else if (q != null) {
	...
	public AudioFormat getEffectiveFormat() {
		...
		map.put(P_QUALITY, Integer.valueOf(getEffectiveQuality()));
		map.put(P_BITRATE, Integer.valueOf(getEffectiveBitRate()));
		map.put(P_CHMODE, chmode2string(getEffectiveChannelMode()));
		map.put(P_VBR, Boolean.valueOf(getEffectiveVBR()));
	...
	private static int string2quality(String q, int def) {
		if (q.equals("lowest")) {
			return QUALITY_LOWEST;
		} else if (q.equals("low")) {
			return QUALITY_LOW;
		} else if (q.equals("middle")) {
			return QUALITY_MIDDLE;
		} else if (q.equals("high")) {
			return QUALITY_HIGH;
		} else if (q.equals("highest")) {
			return QUALITY_HIGHEST;
		}
		return def;
	}
	...
	public static float convertByteArrayToFloat(byte bytes[], int offset, ByteOrder byteOrder)
	{
	...
	public @Nullable AudioFormat getSourceFormat() {
		return sourceFormat;
	}

	public @Nullable AudioFormat getTargetFormat() {
		return targetFormat;
	}
  ...
  private @Nullable AudioFormat sourceFormat;
  private @Nullable AudioFormat targetFormat;
}
