...
public class BitStream {
	 ...
	 */
	public static int getframebits(LameGlobalFlags gfp) {
	 ...
	 */
	private void putbits_noheaders(int val, int j) {
		assert (j < MAX_LENGTH - 2);
	 ...
	 * the ancillary data...
	 */
	private void drain_into_ancillary(LameGlobalFlags gfp,
			int remainingBits) {
		LameInternalFlags gfc = gfp.internal_flags;
		int i;
		...
		if (remainingBits >= 32) {
			String version = Version.getLameShortVersion();
			if (remainingBits >= 32)
	 ...
	 */
	private static void writeheader(LameInternalFlags gfc, int val, int j) {
	...
	public static void CRC_writeheader(LameInternalFlags gfc,
			byte[] header) {
		int crc = 0xffff;
	...
	private static void encodeSideInfo2(LameGlobalFlags gfp,
			int bitsPerFrame) {
		LameInternalFlags gfc = gfp.internal_flags;
		IIISideInfo l3_side;
		int gr, ch;
	...
	private int huffman_coder_count1(LameInternalFlags gfc,
			GrInfo gi) {
		/* Write count1 area */
	 ...
	 */
	private int Huffmancode(LameInternalFlags gfc, int tableindex,
			int start, int end, GrInfo gi) {
	 ...
	 */
	private int ShortHuffmancodebits(LameInternalFlags gfc,
			GrInfo gi) {
	 ...
	 * the buffer. This should be the same as the reservoir BAG_SIZE. Only call this
	 * routine between frames - i.e. only after all headers and data have been
	 * added to the buffer by format_bitstream().
	 * 
	 * Also compute total_bits_output = BAG_SIZE of mp3 buffer (including frame
	 * headers which may not have yet been send to the mp3 buffer) + number of
	 * bits needed to flush all mp3 frames.
	 * 
	 * total_bytes_output is the BAG_SIZE of the mp3 output buffer if
	 * lame_encode_flush_nogap() was called right now.
	 */
	private int compute_flushbits(LameGlobalFlags gfp,
			TotalBytes total_bytes_output) {
		LameInternalFlags gfc = gfp.internal_flags;
		...
		if (flushbits >= 0) {
			...
			/* reduce flushbits by the BAG_SIZE of the headers */
	...
	public void flush_bitstream(LameGlobalFlags gfp) {
		LameInternalFlags gfc = gfp.internal_flags;
		IIISideInfo l3_side;
		int flushbits;
		...
		if (gfc.findReplayGain) {
			float RadioGain = GainAnalysis.GetTitleGain(gfc.rgdata);
	...
	public void add_dummy_byte(LameGlobalFlags gfp, int val,
			int n) {
		LameInternalFlags gfc = gfp.internal_flags;
		int i;
	 ...
	 */
	public int format_bitstream(LameGlobalFlags gfp) {
		LameInternalFlags gfc = gfp.internal_flags;
		IIISideInfo l3_side;
		l3_side = gfc.l3_side;
	 ...
	 * </PRE>
	 */
	public int copy_buffer(LameInternalFlags gfc,
			byte[] buffer, int bufferPos, int size,
			int mp3data) {
		int minimum = bufByteIdx + 1;
		if (minimum <= 0)
			return 0;
		...
		if (mp3data != 0) {
			int[] crc = new int[1];
			crc[0] = gfc.nMusicCRC;
			VBRTag.updateMusicCRC(crc, buffer, bufferPos, minimum);
			gfc.nMusicCRC = crc[0];
			...
			if (gfc.decode_on_the_fly) { /* decode the frame */
				...
				while (samples_out != 0) {
					...
					if (samples_out > 0) {
						...
						if (gfc.findReplayGain)
							if (GainAnalysis.AnalyzeSamples(gfc.rgdata, pcm_buf[0], 0,
	...
	public void init_bit_stream_w(LameInternalFlags gfc) {
