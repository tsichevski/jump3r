...
public class QuantizePVT {
	...
	public QuantizePVT(Takehiro tk) {
		this.tak = tk;
	}

	public float POW20(int x) {
		assert (0 <= (x + QuantizePVT.Q_MAX2) && x < QuantizePVT.Q_MAX);
		return pow20[x + QuantizePVT.Q_MAX2];
	}

	public float IPOW20(int x) {
	 ...
	 * </PRE>
	 */
	private static float ATHmdct(LameGlobalFlags gfp, float f) {
		float ath = PsyModel.ATHformula(f, gfp);
	...
	private static void compute_ath(LameGlobalFlags gfp) {
	 ...
	 */
	public void iteration_init(LameGlobalFlags gfp) {
		LameInternalFlags gfc = gfp.internal_flags;
	 ...
	 */
	public static int on_pe(LameGlobalFlags gfp, float pe[][],
		...
		int extra_bits = Reservoir.ResvMaxBits(gfp, mean_bits, mb, cbr);
		tbits = mb.bits;
	...
	public static void reduce_side(int targ_bits[],
			float ms_ener_ratio, int mean_bits, int max_bits) {
	 ...
	 */
	public static float athAdjust(float a, float x,
			float athFloor) {
		/*
		 * work in progress
		 */
	 ...
	 */
	public static int calc_xmin(LameGlobalFlags gfp,
			III_psy_ratio ratio, GrInfo cod_info,
			float[] pxmin) {
		int pxminPos = 0;
		LameInternalFlags gfc = gfp.internal_flags;
		...
		int enable_athaa_fix = (gfp.VBR == VbrMode.vbr_mtrh) ? 1 : 0;
	...
	private float calc_noise_core(GrInfo cod_info,
			StartLine startline, int l, float step) {
		float noise = 0;
		int j = startline.s;
	 ...
	 * </PRE>
	 */
	public int calc_noise(GrInfo cod_info,
			float[] l3_xmin, float[] distort,
			CalcNoiseResult res, CalcNoiseData prev_noise) {
		int distortPos = 0;
		int l3_xminPos = 0;
		int sfb, l, over = 0;
	 ...
	 */
	private void set_pinfo(LameGlobalFlags gfp,
			GrInfo cod_info, III_psy_ratio ratio,
			int gr, int ch) {
		LameInternalFlags gfc = gfp.internal_flags;
		int sfb, sfb2;
		int l;
	 ...
	 */
	public void set_frame_pinfo(LameGlobalFlags gfp,
			III_psy_ratio ratio[][]) {
		LameInternalFlags gfc = gfp.internal_flags;
