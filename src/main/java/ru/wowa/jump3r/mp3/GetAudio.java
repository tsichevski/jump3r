...
public class GetAudio {
	...
	public GetAudio(Parse parse2, MPGLib mpg2) {
		parse = parse2;
		mpg = mpg2;
	}
	...
	protected static class BlockAlign {
		int offset;
		int blockSize;
	}

	protected static class IFF_AIFF {
		short numChannels;
		int numSampleFrames;
		short sampleSize;
		double sampleRate;
	...
	private MPGLib.@Nullable mpstr_tag hip;

	public void init_infile(LameGlobalFlags gfp,
			String inPath, Enc enc) {
		/* open the input file */
	...
	public void close_infile() {
		closeSndFile(musicin);
	}
	 ...
	 */
	public int get_audio(LameGlobalFlags gfp, int buffer[][]) {
	 ...
	 * sample output
	 */
	public int get_audio16(LameGlobalFlags gfp,
			short buffer[][]) {
	 ...
	 * @return samples read
	 */
	private int get_audio_common(LameGlobalFlags gfp,
			int buffer @Nullable [][],
			short buffer16 @Nullable [][]) {
	...
	int read_samples_mp3(LameGlobalFlags gfp, RandomAccessFile musicin,
			short mpg123pcm[][]) {
		int out;
	...
	public static int WriteWaveHeader(DataOutput fp,
			int pcmbytes, int freq, int channels,
			int bits) {
		try {
			int bytes = (bits + 7) / 8;
	 ...
	 */
	private static int unpack_read_samples(int samples_to_read,
			int bytes_per_sample, boolean swap_order,
			int[] sample_buffer, RandomAccessFile pcm_in)
			throws IOException {
	 ...
	 */
	private int read_samples_pcm(RandomAccessFile musicin,
			int sample_buffer[], int samples_to_read) {
		int samples_read = 0;
	 ...
	 */
	private int parse_wave_header(LameGlobalFlags gfp,
			RandomAccessFile sf) {
		int format_tag = 0;
		int channels = 0;
	...
	private int aiff_check2(IFF_AIFF pcm_aiff_data) {
		...
		if (pcm_aiff_data.blkAlgn.blockSize != 0) {
			if (parse.silent < 10) {
				System.err
						.printf("ERROR: block BAG_SIZE of input sound data is not 0 bytes\n");
			}
			return 1;
		}
		return 0;
	...
	private static long make_even_number_of_bytes_in_length(int x) {
		if ((x & 0x01) != 0) {
			return x + 1;
		}
		return x;
	 ...
	 */
	private int parse_aiff_header(LameGlobalFlags gfp,
			RandomAccessFile sf) {
	 ...
	 */
	private sound_file_format parse_file_header(LameGlobalFlags gfp,
			RandomAccessFile sf) {
	...
	private static void closeSndFile(@Nullable RandomAccessFile musicin) {
		if (musicin != null) {
			try {
				musicin.close();
			} catch (IOException e) {
	...
	private RandomAccessFile OpenSndFile(LameGlobalFlags gfp, String inPath, Enc enc) {
		...
		if (gfp.num_samples == -1) {
			...
			if (flen >= 0) {
				/* try file BAG_SIZE, assume 2 bytes per sample */
				if (is_mpeg_file_format(parse.input_format)) {
  ...
  private static final Charset ISO_8859_1 = Charset.forName("ISO-8859-1");
	...
	private static boolean check_aid(byte[] header) {
	...
	private int lame_decode_initfile(RandomAccessFile fd, MP3Data mp3data, Enc enc) {
		...
		if (mp3data.totalframes > 0) {
			...
			/*
			 * set as unknown. Later, we will take a guess based on file BAG_SIZE
			 * ant bitrate
			 */
			mp3data.nsamp = -1;
		}
	 ...
	 *         on MP3 file.
	 */
	private int lame_decode_fromfile(RandomAccessFile fd,
	...
	private static boolean is_mpeg_file_format(sound_file_format input_file_format) {
	...
	private static int Read32BitsLowHigh(RandomAccessFile fp) {
	...
	private static int Read16BitsLowHigh(RandomAccessFile fp) {
		try {
	...
	private static int Read16BitsHighLow(RandomAccessFile fp) {
		try {
	...
	private static int Read32BitsHighLow(RandomAccessFile fp) {
	...
	private static double unsignedToFloat(double u) {
		return (((long) (u - 2147483647L - 1))) + 2147483648.0;
	}

	private static double ldexp(double x, double exp) {
	 ...
	 */
	private static double convertFromIeeeExtended(byte[] bytes) {
		double f;
	...
	private static double readIeeeExtendedHighLow(RandomAccessFile fp)
			throws IOException {
	...
	private static void write32BitsLowHigh(DataOutput fp, int i)
			throws IOException {
		write16BitsLowHigh(fp, (int) (i & 0xffffL));
		write16BitsLowHigh(fp, (int) ((i >> 16) & 0xffffL));
	}

	public static void write16BitsLowHigh(DataOutput fp, int i)
			throws IOException {
		fp.write(i & 0xff);
		fp.write((i >> 8) & 0xff);
	}
