...
public class FFT {
	...
	private static void fht(float[] fz, int fzPos, int n) {
		int tri = 0;
		int k4;
		int fi;
		int gi;
	...
	public static void fft_short(float x_real[][],
			int chn, float[] buffer[], int bufPos) {
		for (int b = 0; b < 3; b++) {
			int x = Encoder.BLKSIZE_s / 2;
			short k = (short) ((576 / 3) * (b + 1));
	...
	public static void fft_long(float y[], int chn, float[] buffer[],
			int bufPos) {
