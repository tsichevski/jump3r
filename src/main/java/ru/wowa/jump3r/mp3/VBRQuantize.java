...
public class VBRQuantize {
	...
	public void setModules(QuantizePVT qupvt, Takehiro tk) {
		this.qupvt = qupvt;
		this.tak = tk;
	}
	...
	private static float max_x34(float[] xr34, int x34Pos, int bw) {
		float xfsf = 0;
		int j = bw >> 1;
		int remaining = (j & 0x01);
	...
	private int findLowestScalefac(float xr34) {
		...
		int sf = 128, delsf = 64;
		for (int i = 0; i < 8; ++i) {
			float xfsf = qupvt.ipow20[sf] * xr34;
	...
	private static int belowNoiseFloor(float[] xr, int xrPos, float l3xmin,
			int bw) {
		float sum = 0.0f;
	...
	private float calc_sfb_noise_x34(float[] xr, float[] xr34,
			int xrPos, int bw, int sf) {
		double x[] = new double[4];
		int l3[] = new int[4];
		float sfpow = qupvt.pow20[sf + QuantizePVT.Q_MAX2];
		float sfpow34 = qupvt.ipow20[sf];
	...
	private boolean tri_calc_sfb_noise_x34(float[] xr,
			float[] xr34, int xrPos, float l3_xmin,
			int bw, int sf, CalcNoiseCache[] did_it) {
	...
	/**
	 * the find_scalefac* routines calculate a quantization step BAG_SIZE which
	 * would introduce as much noise as is allowed. The larger the step BAG_SIZE the
	 * more quantization noise we'll get. The scalefactors are there to lower
	 * the global step BAG_SIZE, allowing limited differences in quantization step
	 * sizes per band (shaping the noise).
	 */
	private int find_scalefac_x34(float[] xr, float[] xr34,
			int xrPos, float l3_xmin, int bw, int sf_min) {
		...
		for (i = 0; i < 8; ++i) {
			...
			if (sf <= sf_min) {
				...
				boolean bad = tri_calc_sfb_noise_x34(xr, xr34, xrPos,
	 ...
	 */
	private int block_sf(algo_t that, float l3_xmin[],
	...
	private void quantize_x34(algo_t that) {
		...
		while (j <= max_nonzero_coeff) {
			int s = (cod_info.scalefac[sfb] + (cod_info.preflag != 0 ? qupvt.pretab[sfb]
					: 0))
					* ifqstep
					+ cod_info.subblock_gain[cod_info.window[sfb]]
					* 8;
			int sfac = (cod_info.global_gain - s);
			float sfpow34 = qupvt.ipow20[sfac];
	...
	protected static void set_subblock_gain(GrInfo cod_info,
			int mingain_s[], int sf[]) {
		int maxrange1 = 15, maxrange2 = 7;
	...
	protected void set_scalefacs(GrInfo cod_info,
			int[] vbrsfmin, int sf[], int[] max_range) {
		int ifqstep = (cod_info.scalefac_scale == 0) ? 2 : 4;
		int ifqstepShift = (cod_info.scalefac_scale == 0) ? 1 : 2;
		...
		for (int sfb = 0; sfb < sfbmax; ++sfb) {
			int gain = cod_info.global_gain - (sbg[window[sfb]] * 8)
	...
	protected boolean checkScalefactor(GrInfo cod_info,
			int vbrsfmin[]) {
		int ifqstep = cod_info.scalefac_scale == 0 ? 2 : 4;
	...
	private void bitcount(algo_t that) {
		...
		if (that.gfc.mode_gr == 2) {
			...
			rc = tak.scale_bitcount_lsf(that.cod_info);
		}
		if (!rc) {
			return;
		}
	...
	private int quantizeAndCountBits(algo_t that) {
		quantize_x34(that);
		that.cod_info.part2_3_length = Takehiro.noquant_count_bits(that.gfc,
				that.cod_info, null);
		return that.cod_info.part2_3_length;
	}

	private int tryGlobalStepsize(algo_t that, int sfwork[],
			int vbrsfmin[], int delta) {
	...
	private void searchGlobalStepsizeMax(algo_t that, int sfwork[],
			int vbrsfmin[], int target) {
		GrInfo cod_info = that.cod_info;
		int gain = cod_info.global_gain;
		int curr = gain;
		int gain_ok = 1024;
	...
	private static int sfDepth(int sfwork[]) {
		int m = 0;
	...
	private static void cutDistribution(int sfwork[], int sf_out[],
			int cut) {
		for (int j = L3Side.SFBMAX, i = 0; j > 0; --j, ++i) {
			int x = sfwork[i];
	...
	private static int flattenDistribution(int sfwork[], int sf_out[],
			int dm, int k, int p) {
		int sfmax = 0;
		if (dm > 0) {
			for (int j = L3Side.SFBMAX, i = 0; j > 0; --j, ++i) {
	...
	private int tryThatOne(algo_t that, int sftemp[],
			int vbrsfmin[], int vbrmax) {
		float xrpow_max = that.cod_info.xrpow_max;
		int nbits = QuantizePVT.LARGE_BITS;
		that.alloc.alloc(that, sftemp, vbrsfmin, vbrmax);
		bitcount(that);
	...
	private void outOfBitsStrategy(algo_t that, int sfwork[],
			int vbrsfmin[], int target) {
		int wrk[] = new int[L3Side.SFBMAX];
		int dm = sfDepth(sfwork);
		...
		/* PART 1 */
		{
			...
			int bo = dm;
			for (;;) {
				int sfmax = flattenDistribution(sfwork, wrk, dm, bi, p);
			...
			if (bi_ok >= 0) {
				if (bi != bi_ok) {
					int sfmax = flattenDistribution(sfwork, wrk, dm,
							bi_ok, p);
		...
		/* PART 2: */
		{
			...
			int bo = 255;
			for (;;) {
				int sfmax = flattenDistribution(sfwork, wrk, dm, dm, bi);
				int nbits = tryThatOne(that, wrk, vbrsfmin, sfmax);
				if (nbits <= target) {
					bi_ok = bi;
					bo = bi - 1;
			...
			if (bi_ok >= 0) {
				if (bi != bi_ok) {
					int sfmax = flattenDistribution(sfwork, wrk, dm, dm,
							bi_ok);
					tryThatOne(that, wrk, vbrsfmin, sfmax);
				}
				return;
	...
	private int reduce_bit_usage(LameInternalFlags gfc, int gr,
			int ch) {
	...
	public int VBR_encode_frame(LameInternalFlags gfc,
			float xr34orig[][][], float l3_xmin[][][],
			int max_bits[][]) {
			...
			for (int gr = 0; gr < ngr; ++gr) {
				...
				for (int ch = 0; ch < nch; ++ch) {
					if (use_nbits_ch[gr][ch] > LameInternalFlags.MAX_BITS_PER_CHANNEL) {
						 ...
						 * up to a maximum BAG_SIZE of 4095 bits!!!
						 */
						ok = false;
					}
				}
