...
public class Parse {
  ...
  public Parse(Version ver, ID3Tag id3, Presets pre) {
    this.ver = ver;
    this.id3 = id3;
    this.pre = pre;
  }
  ...
  public GetAudio.@Nullable sound_file_format input_format;
  /**
  ...
  private enum TextEncoding {
    ...
    TENC_LATIN1,
    /**
     * text will be converted from local encoding to UCS-2, as ID3v2 wants it
     */
    TENC_UCS2
  }

  private static void set_id3tag(LameGlobalFlags gfp, int type, String str) {
    switch (type) {
    case 'a':
      ID3Tag.id3tag_set_artist(gfp, str);
      return;
    case 't':
      ID3Tag.id3tag_set_title(gfp, str);
      return;
    case 'l':
      ID3Tag.id3tag_set_album(gfp, str);
      return;
    case 'g':
      ID3Tag.id3tag_set_genre(gfp, str);
      return;
    case 'c':
      ID3Tag.id3tag_set_comment(gfp, str);
      return;
    case 'n':
      ID3Tag.id3tag_set_track(gfp, str);
      return;
    case 'y':
      ID3Tag.id3tag_set_year(gfp, str);
      return;
    case 'v':
      ID3Tag.id3tag_set_fieldvalue(gfp, str);
      return;
    }
  }

  private static void set_id3v2tag(LameGlobalFlags gfp, int type, String str) {
    switch (type) {
    case 'a':
      ID3Tag.id3tag_set_textinfo_ucs2(gfp, "TPE1", str);
      return;
    case 't':
      ID3Tag.id3tag_set_textinfo_ucs2(gfp, "TIT2", str);
      return;
    case 'l':
      ID3Tag.id3tag_set_textinfo_ucs2(gfp, "TALB", str);
      return;
    case 'g':
      ID3Tag.id3tag_set_textinfo_ucs2(gfp, "TCON", str);
      return;
    case 'c':
      ID3Tag.id3tag_set_comment(gfp, null, null, str);
      return;
    case 'n':
      ID3Tag.id3tag_set_textinfo_ucs2(gfp, "TRCK", str);
      return;
    }
  }
   ...
   * @return
   */
  private static void id3_tag(LameGlobalFlags gfp, int type, TextEncoding enc,
      String str) {
    String x = null;
    switch (enc) {
    default:
    ...
    case TENC_RAW:
    case TENC_LATIN1:
      set_id3tag(gfp, type, x);
      break;
    case TENC_UCS2:
      set_id3v2tag(gfp, type, x);
      break;
    }
  }
  ...
  private static void lame_version_print(PrintStream fp) {
    String b = Version.getLameOsBitness();
    String v = Version.getLameVersion();
    String u = Version.getLameUrl();
    int lenb = b.length();
    int lenv = v.length();
    int lenu = u.length();
  ...
  private static void print_license(PrintStream fp) {
   ...
   */
  public static void usage(PrintStream fp, String ProgramName) {
    // print general syntax
    lame_version_print(fp);
    fp.printf("usage: %s [options] <infile> [outfile]\n" + "\n"
   ...
   */
  private static void short_help(LameGlobalFlags gfp, PrintStream fp,
      String ProgramName) {
  ...
  private static void long_help(LameGlobalFlags gfp, PrintStream fp,
      String ProgramName) {
    ...
    fp.printf(
        ...
        /**
         * OBSOLETE " --athaa-loudapprox n n=1 total energy or n=2 equal
         * loudness curve\n"
         */
    ...
    fp.printf("    --add-id3v2     force addition of version 2 tag\n"
        ...
        + "    --pad-id3v2     same as '--pad-id3v2-BAG_SIZE 128'\n"
        + "    --pad-id3v2-BAG_SIZE <value> adds version 2 tag, pad with extra <value> bytes\n"
  ...
  public static void display_bitrates(PrintStream fp) {
  ...
  @SuppressWarnings("boxing")
  private static void display_bitrate(PrintStream fp, String version, int d,
      int indx) {
    int nBitrates = 14;
    if (d == 4)
      nBitrates = 8;
   ...
   */
  private static void presets_longinfo_dm(PrintStream msgfp) {
    msgfp
  ...
  @SuppressWarnings("null")
  private static int presets_set(LameGlobalFlags gfp, int fast, int cbr,
      String preset_name, String ProgramName) {
    int mono = 0;
    ...
    if (preset_name.equals("standard")) {
      ...
      Presets.apply_preset(gfp, Lame.INSANE, 1);

      return 0;
    }

    /* Generic ABR Preset */
    if (((Integer.parseInt(preset_name)) > 0) && (fast < 1)) {
      if ((Integer.parseInt(preset_name)) >= 8
          && (Integer.parseInt(preset_name)) <= 320) {
        gfp.preset = Integer.parseInt(preset_name);
        Presets.apply_preset(gfp, Integer.parseInt(preset_name), 1);
   ...
   * the file type. Trying to analyze the file contents is well beyond the scope
   * of LAME and should not be added.
   */
  @SuppressWarnings("null")
  private static sound_file_format filename_to_type(String fileName) {
  ...
  private static int resample_rate(double freq) {
    if (freq >= 1.e3)
      freq *= 1.e-3;
  ...
  private static int set_id3_albumart(LameGlobalFlags gfp, String file_name) {
    int ret = -1;
    ...
    try {
      ...
      try {
        ...
        ret = ID3Tag.id3tag_set_albumart(gfp, albumart, size) ? 0 : 4;
   ...
   * 
   * @param gfp the flags to update
   * @param argv the input argument array
   * @param inPath
   * @param outPath
   * @param nogap_inPath
   * @param ng
   ...
   */
  @SuppressWarnings("null")
  public int parse_args(LameGlobalFlags gfp, ArrayList<String> argv,
      StringBuilder inPath, StringBuilder outPath, String[] nogap_inPath,
      NoGap ng) {
      ...
      if (token.charAt(tokenPos++) == '-') {
        ...
        if (token.length() - tokenPos == 0) {
          ...
          if (inPath.length() == 0) {
            ...
            outPath.setLength(0);
            outPath.append(token);
          }
        }
        if (token.charAt(tokenPos) == '-') { /* GNU style */
          tokenPos++;

          String tokenSubstring = token.substring(tokenPos);
          if (tokenSubstring.equalsIgnoreCase("resample")) {
            argUsed = 1;
            gfp.out_samplerate = resample_rate(Double.parseDouble(nextArg));

          } else if (tokenSubstring.equalsIgnoreCase("vbr-old")) {
            gfp.VBR = VbrMode.vbr_rh;

          } else if (tokenSubstring.equalsIgnoreCase("vbr-new")) {
            gfp.VBR = VbrMode.vbr_mtrh;

          } else if (tokenSubstring.equalsIgnoreCase("vbr-mtrh")) {
            gfp.VBR = VbrMode.vbr_mtrh;

          } else if (tokenSubstring.equalsIgnoreCase("cbr")) {
            gfp.VBR = VbrMode.vbr_off;

          } else if (tokenSubstring.equalsIgnoreCase("abr")) {
            argUsed = 1;
            gfp.VBR = VbrMode.vbr_abr;
            gfp.VBR_mean_bitrate_kbps = Integer.parseInt(nextArg);
            /*
             * values larger than 8000 are bps (like Fraunhofer), so it's
             * strange to get 320000 bps MP3 when specifying 8000 bps MP3
             */
            if (gfp.VBR_mean_bitrate_kbps >= 8000)
              gfp.VBR_mean_bitrate_kbps = (gfp.VBR_mean_bitrate_kbps + 500)
                  / 1000;
          ...
          } else if (tokenSubstring.equalsIgnoreCase("r3mix")) {
            gfp.preset = Lame.R3MIX;
            Presets.apply_preset(gfp, Lame.R3MIX, 1);

          } else if (tokenSubstring.equalsIgnoreCase("bitwidth")) {
            argUsed = 1;
            in_bitwidth = Integer.parseInt(nextArg);

          } else if (tokenSubstring.equalsIgnoreCase("signed")) {
            in_signed = true;

          } else if (tokenSubstring.equalsIgnoreCase("unsigned")) {
            in_signed = false;

          } else if (tokenSubstring.equalsIgnoreCase("little-endian")) {
            in_endian = ByteOrder.LITTLE_ENDIAN;

          } else if (tokenSubstring.equalsIgnoreCase("big-endian")) {
            in_endian = ByteOrder.BIG_ENDIAN;

          } else if (tokenSubstring.equalsIgnoreCase("mp1input")) {
            input_format = GetAudio.sound_file_format.sf_mp1;

          } else if (tokenSubstring.equalsIgnoreCase("mp2input")) {
            input_format = GetAudio.sound_file_format.sf_mp2;

          } else if (tokenSubstring.equalsIgnoreCase("mp3input")) {
            input_format = GetAudio.sound_file_format.sf_mp3;

          } else if (tokenSubstring.equalsIgnoreCase("phone")) {
            if (presets_set(gfp, 0, 0, token, ProgramName) < 0)
              return -1;
            System.err.printf(
                "Warning: --phone is deprecated, use --preset phone instead!");

          } else if (tokenSubstring.equalsIgnoreCase("voice")) {
            if (presets_set(gfp, 0, 0, token, ProgramName) < 0)
              return -1;
            System.err.printf(
              ...
              && tokenSubstring.equalsIgnoreCase("noshort")) {
            gfp.short_blocks = ShortBlock.short_block_dispensed;

          } else if (INTERNAL_OPTS
              && tokenSubstring.equalsIgnoreCase("short")) {
            gfp.short_blocks = ShortBlock.short_block_allowed;

          } else if (INTERNAL_OPTS
              && tokenSubstring.equalsIgnoreCase("allshort")) {
            gfp.short_blocks = ShortBlock.short_block_forced;

          } else if (tokenSubstring.equalsIgnoreCase("decode")) {
            gfp.decode_only = true;

          } else if (tokenSubstring.equalsIgnoreCase("decode-mp3delay")) {
            mp3_delay = Integer.parseInt(nextArg);
            mp3_delay_set = true;
            argUsed = 1;

          } else if (tokenSubstring.equalsIgnoreCase("nores")) {
            gfp.disable_reservoir = true;

          } else if (tokenSubstring.equalsIgnoreCase("strictly-enforce-ISO")) {
            gfp.strict_ISO = true;

          } else if (tokenSubstring.equalsIgnoreCase("scale")) {
            argUsed = 1;
            gfp.scale = (float) Double.parseDouble(nextArg);

          } else if (tokenSubstring.equalsIgnoreCase("scale-l")) {
            argUsed = 1;
            gfp.scale_left = (float) Double.parseDouble(nextArg);

          } else if (tokenSubstring.equalsIgnoreCase("scale-r")) {
            argUsed = 1;
            gfp.scale_right = (float) Double.parseDouble(nextArg);

          } else if (tokenSubstring.equalsIgnoreCase("freeformat")) {
            gfp.free_format = true;

          } else if (tokenSubstring.equalsIgnoreCase("replaygain-fast")) {
            gfp.findReplayGain = true;

          } else if (tokenSubstring.equalsIgnoreCase("replaygain-accurate")) {
            gfp.decode_on_the_fly = true;
            gfp.findReplayGain = true;

          } else if (tokenSubstring.equalsIgnoreCase("noreplaygain")) {
            noreplaygain = 1;
            gfp.findReplayGain = false;

          } else if (tokenSubstring.equalsIgnoreCase("clipdetect")) {
            print_clipping_info = true;
            gfp.decode_on_the_fly = true;

            /* options for ID3 tag */
          } else if (tokenSubstring.equalsIgnoreCase("tt")) {
            argUsed = 1;
            id3_tag(gfp, 't', TextEncoding.TENC_RAW, nextArg);

          } else if (tokenSubstring.equalsIgnoreCase("ta")) {
            argUsed = 1;
            id3_tag(gfp, 'a', TextEncoding.TENC_RAW, nextArg);

          } else if (tokenSubstring.equalsIgnoreCase("tl")) {
            argUsed = 1;
            id3_tag(gfp, 'l', TextEncoding.TENC_RAW, nextArg);

          } else if (tokenSubstring.equalsIgnoreCase("ty")) {
            argUsed = 1;
            id3_tag(gfp, 'y', TextEncoding.TENC_RAW, nextArg);

          } else if (tokenSubstring.equalsIgnoreCase("tc")) {
            argUsed = 1;
            id3_tag(gfp, 'c', TextEncoding.TENC_RAW, nextArg);

          } else if (tokenSubstring.equalsIgnoreCase("tn")) {
            id3_tag(gfp, 'n', TextEncoding.TENC_RAW, nextArg);
            argUsed = 1;
            // if (ret) {
            // if (!ignore_tag_errors) {
            // if (id3tag_mode == ID3TAG_MODE.ID3TAG_MODE_V1_ONLY) {
            // System.err
            // .printf("The track number has to be between 1 and 255 for
            // ID3v1.\n");
            // return -1;
          ...
          } else if (tokenSubstring.equalsIgnoreCase("tg")) {
            id3_tag(gfp, 'g', TextEncoding.TENC_RAW, nextArg);
            argUsed = 1;

          } else if (tokenSubstring.equalsIgnoreCase("tv")) {
            argUsed = 1;
            id3_tag(gfp, 'v', TextEncoding.TENC_RAW, nextArg);
            // if (silent < 10) {
            // System.err.printf(
          ...
          } else if (tokenSubstring.equalsIgnoreCase("ti")) {
            argUsed = 1;
          ...
          } else if (tokenSubstring.equalsIgnoreCase("ignore-tag-errors")) {
            ignore_tag_errors = true;

          } else if (tokenSubstring.equalsIgnoreCase("add-id3v2")) {
            ID3Tag.id3tag_add_v2(gfp);

          } else if (tokenSubstring.equalsIgnoreCase("id3v1-only")) {
            ID3Tag.id3tag_v1_only(gfp);

          } else if (tokenSubstring.equalsIgnoreCase("id3v2-only")) {
            ID3Tag.id3tag_v2_only(gfp);

          } else if (tokenSubstring.equalsIgnoreCase("space-id3v1")) {
            ID3Tag.id3tag_space_v1(gfp);

          } else if (tokenSubstring.equalsIgnoreCase("pad-id3v2")) {
            ID3Tag.id3tag_pad_v2(gfp);

          } else if (tokenSubstring.equalsIgnoreCase("pad-id3v2-BAG_SIZE")) {
            int n = Integer.parseInt(nextArg);
            n = n <= 128000 ? n : 128000;
            n = n >= 0 ? n : 0;
            ID3Tag.id3tag_set_pad(gfp, n);
            argUsed = 1;

          } else if (tokenSubstring.equalsIgnoreCase("genre-list")) {
            ID3Tag.id3tag_genre_list(new GenreListHandler() {

              @Override
              public void genre_list_handler(int num, String name) {
                System.out.printf("%3d %s\n", Integer.valueOf(num), name);
              }
          ...
          } else if (tokenSubstring.equalsIgnoreCase("lowpass")) {
          ...
          } else if (tokenSubstring.equalsIgnoreCase("lowpass-width")) {
            val = Double.parseDouble(nextArg);
            argUsed = 1;
          ...
          } else if (tokenSubstring.equalsIgnoreCase("highpass")) {
            val = Double.parseDouble(nextArg);
            argUsed = 1;
            if (val < 0.0) {
              gfp.highpassfreq = -1;
          ...
          } else if (tokenSubstring.equalsIgnoreCase("highpass-width")) {
            val = Double.parseDouble(nextArg);
            argUsed = 1;
            /* useful are 0.001 kHz...16 kHz, 16 Hz...50000 Hz */
            if (val < 0.001 || val > 50000.) {
          ...
          } else if (tokenSubstring.equalsIgnoreCase("comp")) {
            argUsed = 1;
            val = Double.parseDouble(nextArg);
            if (val < 1.0) {
          ...
          } else if (tokenSubstring.equalsIgnoreCase("notemp")) {
            gfp.useTemporal = Boolean.FALSE;

          } else if (tokenSubstring.equalsIgnoreCase("interch")) {
            argUsed = 1;
            gfp.interChRatio = (float) Double.parseDouble(nextArg);

          } else if (tokenSubstring.equalsIgnoreCase("temporal-masking")) {
            argUsed = 1;
            gfp.useTemporal = Boolean.valueOf(Integer.parseInt(nextArg) != 0);

          } else if (tokenSubstring.equalsIgnoreCase("nssafejoint")) {
            gfp.exp_nspsytune = gfp.exp_nspsytune | 2;

          } else if (tokenSubstring.equalsIgnoreCase("nsmsfix")) {
            argUsed = 1;
            gfp.msfix = (float) Double.parseDouble(nextArg);

          } else if (tokenSubstring.equalsIgnoreCase("ns-bass")) {
            argUsed = 1;
            {
              double d;
              int k;
          ...
          } else if (tokenSubstring.equalsIgnoreCase("ns-alto")) {
            argUsed = 1;
            {
              double d;
              int k;
          ...
          } else if (tokenSubstring.equalsIgnoreCase("ns-treble")) {
            argUsed = 1;
            {
              double d;
              int k;
          ...
          } else if (tokenSubstring.equalsIgnoreCase("ns-sfb21")) {
            /*
             * to be compatible with Naoki's original code, ns-sfb21 specifies
             * how to change ns-treble for sfb21
             */
            argUsed = 1;
            {
              double d;
          ...
          } else if (tokenSubstring.equalsIgnoreCase("quiet")
              || tokenSubstring.equalsIgnoreCase("silent")) {
            silent = 10; /* on a scale from 1 to 10 be very silent */

          } else if (tokenSubstring.equalsIgnoreCase("brief")) {
            silent = -5; /* print few info on screen */

          } else if (tokenSubstring.equalsIgnoreCase("verbose")) {
            silent = -10; /* print a lot on screen */

          } else if (tokenSubstring.equalsIgnoreCase("version")
              || tokenSubstring.equalsIgnoreCase("license")) {
            print_license(System.out);
            return -2;

          } else if (tokenSubstring.equalsIgnoreCase("help")
              || tokenSubstring.equalsIgnoreCase("usage")) {
            short_help(gfp, System.out, ProgramName);
            return -2;

          } else if (tokenSubstring.equalsIgnoreCase("longhelp")) {
            long_help(gfp, System.out, ProgramName);
            return -2;

          } else if (tokenSubstring.equalsIgnoreCase("?")) {
            long_help(gfp, System.out, ProgramName);
            return -2;

          } else if (tokenSubstring.equalsIgnoreCase("preset")
              || tokenSubstring.equalsIgnoreCase("alt-preset")) {
            argUsed = 1;
            {
              int fast = 0, cbr = 0;
          ...
          } else if (tokenSubstring.equalsIgnoreCase("disptime")) {
            argUsed = 1;
            update_interval = (float) Double.parseDouble(nextArg);

          } else if (tokenSubstring.equalsIgnoreCase("nogaptags")) {
            nogap_tags = 1;

          } else if (tokenSubstring.equalsIgnoreCase("nogapout")) {
            outPath.setLength(0);
            outPath.append(nextArg);
            argUsed = 1;

          } else if (tokenSubstring.equalsIgnoreCase("nogap")) {
            nogap = 1;

          } else if (INTERNAL_OPTS && tokenSubstring
              .equalsIgnoreCase("tune")) { /* without helptext */
            argUsed = 1;
            {
              ...
              && tokenSubstring.equalsIgnoreCase("shortthreshold")) {
            {
              float x, y;
              ...
              .equalsIgnoreCase("maskingadjust")) { /* without helptext */
            argUsed = 1;
            gfp.maskingadjust = (float) Double.parseDouble(nextArg);

          } else if (INTERNAL_OPTS && tokenSubstring
              .equalsIgnoreCase("maskingadjustshort")) { /* without helptext */
            argUsed = 1;
            gfp.maskingadjust_short = (float) Double.parseDouble(nextArg);

          } else if (INTERNAL_OPTS && tokenSubstring
              .equalsIgnoreCase("athcurve")) { /* without helptext */
            argUsed = 1;
            gfp.ATHcurve = (float) Double.parseDouble(nextArg);

          } else if (INTERNAL_OPTS && tokenSubstring
              .equalsIgnoreCase("no-preset-tune")) { /* without helptext */

          } else if (INTERNAL_OPTS
              && tokenSubstring.equalsIgnoreCase("substep")) {
            argUsed = 1;
            gfp.internal_flags.substep_shaping = Integer.parseInt(nextArg);

          } else if (INTERNAL_OPTS && tokenSubstring
              .equalsIgnoreCase("sbgain")) { /* without helptext */
            argUsed = 1;
            gfp.internal_flags.subblock_gain = Integer.parseInt(nextArg);

          } else if (INTERNAL_OPTS && tokenSubstring
              .equalsIgnoreCase("sfscale")) { /* without helptext */
            gfp.internal_flags.noise_shaping = 2;

          } else if (INTERNAL_OPTS
              && tokenSubstring.equalsIgnoreCase("noath")) {
            gfp.noATH = true;

          } else if (INTERNAL_OPTS
              && tokenSubstring.equalsIgnoreCase("athonly")) {
            gfp.ATHonly = true;

          } else if (INTERNAL_OPTS
              && tokenSubstring.equalsIgnoreCase("athshort")) {
            gfp.ATHshort = true;

          } else if (INTERNAL_OPTS
              && tokenSubstring.equalsIgnoreCase("athlower")) {
            argUsed = 1;
            gfp.ATHlower = -(float) Double.parseDouble(nextArg) / 10.0f;

          } else if (INTERNAL_OPTS
              && tokenSubstring.equalsIgnoreCase("athtype")) {
            argUsed = 1;
            gfp.ATHtype = Integer.parseInt(nextArg);

          } else if (INTERNAL_OPTS
              && tokenSubstring.equalsIgnoreCase("athaa-type")) {
            /*
             ...
             */
            gfp.athaa_type = Integer.parseInt(nextArg);
            /*
             * now: 0:off else:Jon
             */

          } else if (tokenSubstring.equalsIgnoreCase("athaa-sensitivity")) {
            argUsed = 1;
          ...
          while (tokenPos < token.length()) {
            ...
            case 'V':
              ...
              gfp.VBR_q_frac = (float) Double.parseDouble(arg) - gfp.VBR_q;
              break;
            case 'v':
              /* to change VBR default look in lame.h */
              if (gfp.VBR == VbrMode.vbr_off)
            ...
            case 'q':
              argUsed = 1; {
              int tmp_quality = Integer.parseInt(arg);
            ...
            case 's':
              ...
              gfp.in_samplerate = (int) (val * (val <= 192 ? 1.e3 : 1.e0)
                  + 0.5);
              break;
            case 'b':
              argUsed = 1;
              gfp.brate = Integer.parseInt(arg);
            ...
            case 'B':
              argUsed = 1;
              gfp.VBR_max_bitrate_kbps = Integer.parseInt(arg);
              break;
            case 'F':
              gfp.VBR_hard_min = 1;
              break;
            ...
            case 'p':
              /*
               * (jo) error_protection: add crc16 information to stream
               */
              gfp.error_protection = true;
              break;
            case 'a':
              /*
               * autoconvert input file from stereo to mono - for mono mp3
               * encoding
               */
              autoconvert = 1;
              gfp.mode = MPEGMode.MONO;
            ...
            case 'X':
            /*
             * experimental switch -X: the differnt types of quant compare are
             * tough to communicate to endusers, so they shouldn't bother to toy
             * around with them
             */
            {
              int x, y;
