...
public class Lame {
	...
	public Lame(GainAnalysis ga, BitStream bs, Presets p,
	...
	/**
	 * maximum BAG_SIZE of albumart image (128KB), which affects LAME_MAXMP3BUFFER
	 * as well since lame_encode_buffer() also returns ID3v2 tag data
	 */
	static final int LAME_MAXALBUMART = (128 * 1024);

	/**
	 * maximum BAG_SIZE of mp3buffer needed if you encode at most 1152 samples for
	...
	private static float filter_coef(float x) {
		if (x > 1.0)
			return 0.0f;
		if (x <= 0.0)
			return 1.0f;
	...
	private static void lame_init_params_ppflt(LameGlobalFlags gfp) {
	...
	private static void optimum_bandwidth(LowPassHighPass lh, int bitrate) {
		/**
		 * <PRE>
		 *  Input:
		 ...
		 * </PRE>
		 */
		BandPass freq_map[] = new BandPass[] { new BandPass(2000),
				new BandPass(3700), new BandPass(3900),
				new BandPass(5500), new BandPass(7000),
				new BandPass(7500), new BandPass(10000),
				new BandPass(11000), new BandPass(13500),
				new BandPass(15100), new BandPass(15600),
				new BandPass(17000), new BandPass(17500),
				new BandPass(18600), new BandPass(19400),
				new BandPass(19700), new BandPass(20500) };

		int table_index = nearestBitrateFullIndex(bitrate);
		lh.lowerlimit = freq_map[table_index].lowpass;
	}

	private static int optimum_samplefreq(int lowpassfreq,
	 ...
	 */
	private static void lame_init_qval(LameGlobalFlags gfp) {
		LameInternalFlags gfc = gfp.internal_flags;
	...
	private static double linear_int(double a, double b, double m) {
		return a + m * (b - a);
	}
	 ...
	 */
	private static int FindNearestBitrate(int bRate, int version,
			int samplerate) {
	 ...
	 */
	public static int nearestBitrateFullIndex(int bitrate) {
		/* borrowed from DM abr presets */

		int full_bitrate_table[] = { 8, 16, 24, 32, 40, 48, 56, 64, 80,
	 ...
	 */
	private static int map2MP3Frequency(int freq) {
		if (freq <= 8000)
			return 8000;
		if (freq <= 11025)
			return 11025;
	 ...
	 */
	private static int SmpFrqIndex(int sample_freq, LameGlobalFlags gpf) {
		switch (sample_freq) {
		case 44100:
			gpf.version = 1;
			return 0;
	 ...
	 */
	public static int BitrateIndex(int bRate, int version,
			int samplerate) {
	 ...
	 */
	private static float blackman(float x, float fcn, int l) {
		/*
	...
	private int fill_buffer_resample(LameGlobalFlags gfp,
			float[] outbuf, int outbufPos, int desired_len,
			float[] inbuf, int in_bufferPos, int len,
			NumUsed num_used, int ch) {
		LameInternalFlags gfc = gfp.internal_flags;
		int i, j = 0, k;
		...
		int BLACKSIZE = filter_l + 1; /* BAG_SIZE of data needed for FIR */
	...
	private void fill_buffer(LameGlobalFlags gfp, float mfbuf[][],
			float in_buffer[][], int in_bufferPos,
			int nsamples, InOut io) {
		LameInternalFlags gfc = gfp.internal_flags;
	 ...
	 * 
	 ********************************************************************/
	public int lame_init_params(LameGlobalFlags gfp) {
		LameInternalFlags gfc = gfp.internal_flags;
		...
		if (gfp.lowpassfreq == 0) {
			...
			case vbr_rh: {
				int x[] = { 19500, 19000, 18600, 18000, 17500, 16000,
			...
			default: {
				int x[] = { 19500, 19000, 18500, 18000, 17500, 16500,
		...
		if (gfc.findReplayGain) {
			if (GainAnalysis.InitGainAnalysis(gfc.rgdata, gfp.out_samplerate) == GainAnalysis.INIT_GAIN_ANALYSIS_ERROR) {
		...
		case vbr_mtrh: {
			...
			float cmp[] = { 5.7f, 6.5f, 7.3f, 8.2f, 10f, 11.9f, 13f, 14f,
					15f, 16.5f };
		...
		for (int i = 0; i < Encoder.PSFB21 + 1; i++) {
			int size = (gfc.scalefac_band.l[22] - gfc.scalefac_band.l[21])
					/ Encoder.PSFB21;
		...
		for (int i = 0; i < Encoder.PSFB12 + 1; i++) {
			int size = (gfc.scalefac_band.s[13] - gfc.scalefac_band.s[12])
					/ Encoder.PSFB12;
		...
		case vbr_mtrh: {
			...
			Presets.apply_preset(gfp, 500 - (gfp.VBR_q * 10), 0);
			/**
			 * <PRE>
		...
		default: /* cbr/abr */{
			...
			Presets.apply_preset(gfp, gfp.VBR_mean_bitrate_kbps, 0);
			gfp.VBR = vbrmode;
	 ...
	 */
	public static void lame_print_internals(LameGlobalFlags gfp) {
		LameInternalFlags gfc = gfp.internal_flags;
	 ...
	 * program.
	 */
	private int lame_encode_frame(LameGlobalFlags gfp,
			float inbuf_l[], float inbuf_r[], byte[] mp3buf,
	...
	private static void update_inbuffer_size(LameInternalFlags gfc,
			int nsamples) {
	...
	private static int calcNeeded(LameGlobalFlags gfp) {
	 ...
	 * </PRE>
	 */
	private int lame_encode_buffer_sample(LameGlobalFlags gfp,
			float buffer_l[], float buffer_r[], int nsamples,
			byte[] mp3buf, int mp3bufPos, int mp3buf_size) {
		LameInternalFlags gfc = gfp.internal_flags;
		...
		while (nsamples > 0) {
			...
			if (gfc.findReplayGain && !gfc.decode_on_the_fly)
				if (GainAnalysis.AnalyzeSamples(gfc.rgdata, mfbuf[0], gfc.mf_size,
			...
			if (gfc.mf_size >= mf_needed) {
				...
				/* mp3buf_size = BAG_SIZE of original mp3 output buffer */
				/* = 0 if we should not worry about the */
				/* buffer BAG_SIZE because calling program is */
				/* to lazy to compute it */
				/* mp3size = BAG_SIZE of data written to buffer so far */
	...
	private int lame_encode_buffer(LameGlobalFlags gfp,
			short buffer_l[], short buffer_r[], int nsamples,
			byte[] mp3buf, int mp3bufPos, int mp3buf_size) {
		LameInternalFlags gfc = gfp.internal_flags;
		float in_buffer[][] = new float[2][];
	...
	public int lame_encode_buffer_int(LameGlobalFlags gfp,
			int buffer_l[], int buffer_r[], int nsamples,
			byte[] mp3buf, int mp3bufPos, int mp3buf_size) {
		LameInternalFlags gfc = gfp.internal_flags;
		float[][] in_buffer = new float[2][];
	 ...
	 * Reset reservoir BAG_SIZE to 0 but keep all PCM samples and MDCT data in
	 ...
	 */
	public int lame_encode_flush_nogap(LameGlobalFlags gfp,
			byte[] mp3buffer, int mp3buffer_size) {
		LameInternalFlags gfc = gfp.internal_flags;
	 ...
	 */
	public void lame_init_bitstream(LameGlobalFlags gfp) {
		LameInternalFlags gfc = gfp.internal_flags;
		gfp.frameNum = 0;
	 ...
	 * tags into bitstream.
	 */
	public int lame_encode_flush(LameGlobalFlags gfp,
			byte[] mp3buffer, int mp3bufferPos, int mp3buffer_size) {
		LameInternalFlags gfc = gfp.internal_flags;
		...
		while (frames_left > 0 && imp3 >= 0) {
			...
			/* if user specifed buffer BAG_SIZE = 0, dont check BAG_SIZE */
			if (mp3buffer_size == 0)
	 ...
	 */
	public static int lame_close(LameGlobalFlags gfp) {
		int ret = 0;
	...
	private static void lame_init_old(LameGlobalFlags gfp) {
	...
	public static LameGlobalFlags lame_init() {
	 ...
	 * </PRE>
	 */
	public static void lame_bitrate_kbps(LameGlobalFlags gfp,
			int bitrate_kbps[]) {
		if (gfp.free_format) {
			for (int i = 0; i < 14; i++)
				bitrate_kbps[i] = -1;
	...
	public static void lame_bitrate_hist(LameGlobalFlags gfp,
			int bitrate_count[]) {
		LameInternalFlags gfc = gfp.internal_flags;
		if (gfp.free_format) {
			for (int i = 0; i < 14; i++)
	...
	public static void lame_stereo_mode_hist(LameGlobalFlags gfp,
			int stmode_count[]) {
		LameInternalFlags gfc = gfp.internal_flags;
		for (int i = 0; i < 4; i++) {
	...
	public static void lame_bitrate_stereo_mode_hist(LameGlobalFlags gfp,
	...
	public static void lame_block_type_hist(LameGlobalFlags gfp,
			int btype_count[]) {
		LameInternalFlags gfc = gfp.internal_flags;
		for (int i = 0; i < 6; ++i) {
	...
	public static void lame_bitrate_block_type_hist(LameGlobalFlags gfp,
