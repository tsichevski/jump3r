...
public class Quantize {
	...
	public Quantize(BitStream bs, Reservoir rv, QuantizePVT qupvt, Takehiro tk) {
		this.bs = bs;
		this.rv = rv;
		this.qupvt = qupvt;
		this.tk = tk;
	 ...
	 */
	public static void ms_convert(IIISideInfo l3_side, int gr) {
		for (int i = 0; i < 576; ++i) {
	 ...
	 */
	private static float init_xrpow_core(GrInfo cod_info, float xrpow[],
			int upper, float sum) {
		sum = 0;
	...
	public static boolean init_xrpow(LameInternalFlags gfc,
			GrInfo cod_info, float xrpow[]) {
		float sum = 0;
	 ...
	 */
	private static void psfb21_analogsilence(LameInternalFlags gfc,
			GrInfo cod_info) {
		ATH ath = gfc.ATH;
		...
		if (cod_info.block_type != Encoder.SHORT_TYPE) {
			...
			for (int gsfb = Encoder.PSFB21 - 1; gsfb >= 0 && !stop; gsfb--) {
				...
				float ath21 = QuantizePVT.athAdjust(ath.adjust, ath.psfb21[gsfb],
						ath.floor);
			...
			for (int block = 0; block < 3; block++) {
				...
				for (int gsfb = Encoder.PSFB12 - 1; gsfb >= 0 && !stop; gsfb--) {
					int start = gfc.scalefac_band.s[12]
							* 3
					...
					float ath12 = QuantizePVT.athAdjust(ath.adjust, ath.psfb12[gsfb],
							ath.floor);
	...
	public void init_outer_loop(LameInternalFlags gfc,
			GrInfo cod_info) {
		/*
	 ...
	 * author/date??
	 * 
	 * binary step BAG_SIZE search used by outer_loop to get a quantizer step BAG_SIZE
	 * to start with
	 */
	private int bin_search_StepSize(LameInternalFlags gfc,
			GrInfo cod_info, int desired_rate, int ch,
			float xrpow[]) {
		int nBits;
	...
	public void trancate_smallspectrums(LameInternalFlags gfc,
			GrInfo gi, float[] l3_xmin, float[] work) {
	 ...
	 */
	private static boolean loop_break(GrInfo cod_info) {
	...
	private static double penalties(double noise) {
	 ...
	 */
	private static double get_klemm_noise(float[] distort, GrInfo gi) {
	...
	private static boolean quant_compare(int quant_comp,
			CalcNoiseResult best, CalcNoiseResult calc,
			GrInfo gi, float[] distort) {
		/**
		...
		if (best.over_count == 0) {
			 ...
			 * is sometimes a poor estimator of the final BAG_SIZE at low bitrates.
			 */
	 ...
	 * </PRE>
	 */
	private static void amp_scalefac_bands(LameGlobalFlags gfp,
			GrInfo cod_info, float[] distort,
			float xrpow[], boolean bRefine) {
	...
	private void inc_scalefac_scale(GrInfo cod_info, float xrpow[]) {
		float ifqstep34 = 1.29683955465100964055f;
	 ...
	 */
	private boolean inc_subblock_gain(LameInternalFlags gfc,
			GrInfo cod_info, float xrpow[]) {
		int sfb;
	 ...
	 * </PRE>
	 */
	private boolean balance_noise(LameGlobalFlags gfp,
			GrInfo cod_info, float[] distort, float xrpow[],
			boolean bRefine) {
		LameInternalFlags gfc = gfp.internal_flags;
		...
		if (gfc.mode_gr == 2)
			...
			status = tk.scale_bitcount_lsf(cod_info);
	 ...
	 */
	public int outer_loop(LameGlobalFlags gfp,
			GrInfo cod_info, float[] l3_xmin, float xrpow[],
			int ch, int targ_bits) {
		LameInternalFlags gfc = gfp.internal_flags;
	 ...
	 * 
	 * update reservoir status after quantization/bitrate
	 */
	public void iteration_finish_one(LameInternalFlags gfc,
			int gr, int ch) {
	 ...
	 */
	public void VBR_encode_granule(LameGlobalFlags gfp,
			GrInfo cod_info, float[] l3_xmin, float xrpow[],
			int ch, int min_bits, int max_bits) {
		LameInternalFlags gfc = gfp.internal_flags;
	 ...
	 */
	public static void get_framebits(LameGlobalFlags gfp,
			int frameBits[]) {
		LameInternalFlags gfc = gfp.internal_flags;
		...
		int bitsPerFrame = BitStream.getframebits(gfp);

		/*
		 * bits for analog silence
		 */
		gfc.bitrate_index = 1;
		bitsPerFrame = BitStream.getframebits(gfp);

		for (int i = 1; i <= gfc.VBR_max_bitrate; i++) {
			gfc.bitrate_index = i;
			MeanBits mb = new MeanBits(bitsPerFrame);
			frameBits[i] = Reservoir.ResvFrameBegin(gfp, mb);
	 ...
	 * </PRE>
	 */
	public int VBR_old_prepare(LameGlobalFlags gfp, float pe[][],
			float ms_ener_ratio[], III_psy_ratio ratio[][],
			float l3_xmin[][][], int frameBits[], int min_bits[][],
			int max_bits[][], int bands[][]) {
		LameInternalFlags gfc = gfp.internal_flags;
		...
		int avg = Reservoir.ResvFrameBegin(gfp, new MeanBits(0)) / gfc.mode_gr;

		get_framebits(gfp, frameBits);

		for (int gr = 0; gr < gfc.mode_gr; gr++) {
			int mxb = QuantizePVT.on_pe(gfp, pe, max_bits[gr], avg, gr, 0);
			...
			for (int ch = 0; ch < gfc.channels_out; ++ch) {
				...
				bands[gr][ch] = QuantizePVT.calc_xmin(gfp, ratio[gr][ch], cod_info,
						l3_xmin[gr][ch]);
				if (bands[gr][ch] != 0)
					analog_silence = 0;
	...
	public static void bitpressure_strategy(LameInternalFlags gfc,
			float l3_xmin[][][], int min_bits[][], int max_bits[][]) {
		for (int gr = 0; gr < gfc.mode_gr; gr++) {
			for (int ch = 0; ch < gfc.channels_out; ch++) {
	...
	public int VBR_new_prepare(LameGlobalFlags gfp, float pe[][],
			III_psy_ratio ratio[][], float l3_xmin[][][],
			int frameBits[], int max_bits[][]) {
		LameInternalFlags gfc = gfp.internal_flags;
		...
		if (!gfp.free_format) {
			...
			gfc.bitrate_index = 0;
			MeanBits mb = new MeanBits(avg);
			maximum_framebits = Reservoir.ResvFrameBegin(gfp, mb);
			avg = mb.bits;
			frameBits[0] = maximum_framebits;
		}

		for (int gr = 0; gr < gfc.mode_gr; gr++) {
			QuantizePVT.on_pe(gfp, pe, max_bits[gr], avg, gr, 0);
			if (gfc.mode_ext == Encoder.MPG_MD_MS_LR) {
				ms_convert(gfc.l3_side, gr);
			}
			for (int ch = 0; ch < gfc.channels_out; ++ch) {
				...
				init_outer_loop(gfc, cod_info);
				if (0 != QuantizePVT.calc_xmin(gfp, ratio[gr][ch], cod_info,
						l3_xmin[gr][ch]))
					analog_silence = 0;
	 ...
	 * mt 2000/05/31
	 */
	public static void calc_target_bits(LameGlobalFlags gfp, float pe[][],
			float ms_ener_ratio[], int targ_bits[][],
		...
		max_frame_bits[0] = Reservoir.ResvFrameBegin(gfp, mb);
		mean_bits = mb.bits;

		gfc.bitrate_index = 1;
		mean_bits = BitStream.getframebits(gfp) - gfc.sideinfo_len * 8;
		...
		if (gfc.mode_ext == Encoder.MPG_MD_MS_LR)
			for (gr = 0; gr < gfc.mode_gr; gr++) {
				QuantizePVT.reduce_side(targ_bits[gr], ms_ener_ratio[gr], mean_bits
						* gfc.channels_out,
