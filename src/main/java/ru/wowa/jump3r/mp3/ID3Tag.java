...
public class ID3Tag {
  ...
  public ID3Tag(BitStream bits, Version ver) {
    this.bits = bits;
    this.ver = ver;
  }
  ...
  public void setModules(BitStream bits, Version ver) {
    this.bits = bits;
    this.ver = ver;
  }
  ...
  private static final int genre_alpha_map[] = { 123, 34, 74, 73, 99, 20, 40,
      26, 145, 90, 116, 41, 135, 85, 96, 138, 89, 0, 107, 132, 65, 88, 104, 102,
      97, 136, 61, 141, 32, 1, 112, 128, 57, 140, 2, 139, 58, 3, 125, 50, 22, 4,
      55, 127, 122, 120, 98, 52, 48, 54, 124, 25, 84, 80, 115, 81, 119, 5, 30,
      36, 59, 126, 38, 49, 91, 6, 129, 79, 137, 7, 35, 100, 131, 19, 33, 46, 47,
      8, 29, 146, 63, 86, 71, 45, 142, 9, 77, 82, 64, 133, 10, 66, 39, 11, 103,
      12, 75, 134, 13, 53, 62, 109, 117, 23, 108, 92, 67, 93, 43, 121, 15, 68,
      14, 16, 76, 87, 118, 17, 78, 143, 114, 110, 69, 21, 111, 95, 105, 42, 37,
      24, 56, 44, 101, 83, 94, 106, 147, 113, 18, 51, 130, 144, 60, 70, 31, 72,
      27, 28 };
  ...
  private static void copyV1ToV2(LameGlobalFlags gfp, int frame_id, String s) {
  ...
  private static void id3v2AddAudioDuration(LameGlobalFlags gfp) {
    if (gfp.num_samples != -1) {
      ...
      @NonNull
      String buffer = String.format("%d", Long.valueOf(playlength_ms));
      copyV1ToV2(gfp, ID_PLAYLENGTH, buffer);
    }
  }

  @SuppressWarnings("null")
  public static void id3tag_genre_list(@Nullable GenreListHandler handler) {
    if (handler != null) {
  ...
  public static void id3tag_init(LameGlobalFlags gfp) {
    LameInternalFlags gfc = gfp.internal_flags;
    ...
    @SuppressWarnings("null")
    @NonNull
    String buffer = lenb > 0 ? String.format("LAME %s version %s (%s)", b, v, u)
        : String.format("LAME version %s (%s)", v, u);

    copyV1ToV2(gfp, ID_ENCODER, buffer);
  }

  public static void id3tag_add_v2(LameGlobalFlags gfp) {
    LameInternalFlags gfc = gfp.internal_flags;
    gfc.tag_spec.flags &= ~V1_ONLY_FLAG;
    gfc.tag_spec.flags |= ADD_V2_FLAG;
  }

  public static void id3tag_v1_only(LameGlobalFlags gfp) {
    LameInternalFlags gfc = gfp.internal_flags;
    gfc.tag_spec.flags &= ~(ADD_V2_FLAG | V2_ONLY_FLAG);
    gfc.tag_spec.flags |= V1_ONLY_FLAG;
  }

  public static void id3tag_v2_only(LameGlobalFlags gfp) {
    LameInternalFlags gfc = gfp.internal_flags;
    gfc.tag_spec.flags &= ~V1_ONLY_FLAG;
    gfc.tag_spec.flags |= V2_ONLY_FLAG;
  }

  public static void id3tag_space_v1(LameGlobalFlags gfp) {
    LameInternalFlags gfc = gfp.internal_flags;
    gfc.tag_spec.flags &= ~V2_ONLY_FLAG;
    gfc.tag_spec.flags |= SPACE_V1_FLAG;
  }

  public static void id3tag_pad_v2(LameGlobalFlags gfp) {
    id3tag_set_pad(gfp, 128);
  }

  public static void id3tag_set_pad(LameGlobalFlags gfp, int n) {
    LameInternalFlags gfc = gfp.internal_flags;
    assert gfc != null;
    gfc.tag_spec.flags &= ~V1_ONLY_FLAG;
  ...
  public static boolean id3tag_set_albumart(LameGlobalFlags gfp, byte[] image,
      int size) {
    ...
    /* make sure the image BAG_SIZE is no larger than the maximum value */
  ...
  private static int set_4_byte_value(byte[] bytes, int bytesPos, int value) {
    int i;
    for (i = 3; i >= 0; --i) {
  ...
  private static boolean isNumericString(int frame_id) {
  ...
  private static boolean isMultiFrame(int frame_id) {
    if (frame_id == ID_TXXX || frame_id == ID_WXXX || frame_id == ID_COMMENT
        || frame_id == ID_SYLT || frame_id == ID_APIC || frame_id == ID_GEOB
        || frame_id == ID_PCNT || frame_id == ID_AENC || frame_id == ID_LINK
        || frame_id == ID_ENCR || frame_id == ID_GRID || frame_id == ID_PRIV) {
      return true;
    }
    return false;
  }

  private static boolean hasUcs2ByteOrderMarker(char bom) {
  ...
  private static @Nullable FrameDataNode findNode(ID3TagSpec tag, int frame_id,
      @Nullable FrameDataNode last) {
  ...
  @SuppressWarnings("null")
  private static void appendNode(ID3TagSpec tag, FrameDataNode node) {
  ...
  @SuppressWarnings("null")
  private static String setLang(@Nullable String src) {
    int i;
  ...
  private static boolean isSameLang(String l1, @Nullable String l2) {
    String d = setLang(l2);
    for (int i = 0; i < 3; ++i) {
  ...
  @SuppressWarnings("null")
  private static boolean isSameDescriptor(FrameDataNode node,
      @Nullable String dsc) {
  ...
  @SuppressWarnings("null")
  private static boolean isSameDescriptorUcs2(FrameDataNode node,
      @Nullable String dsc) {
    if (node.dsc.enc != 1 && node.dsc.dim > 0) {
      return false;
    }
  ...
  private static void id3v2_add_ucs2(LameGlobalFlags gfp, int frame_id,
      @Nullable String lang, @Nullable String desc, @Nullable String text) {
    LameInternalFlags gfc = gfp.internal_flags;
    @Nullable
  ...
  private static void id3v2_add_latin1(LameGlobalFlags gfp, int frame_id,
      @Nullable String lang, @Nullable String desc, @Nullable String text) {
    @Nullable
    LameInternalFlags gfc = gfp.internal_flags;
    FrameDataNode node;
  ...
  public static int id3tag_set_textinfo_ucs2(@Nullable LameGlobalFlags gfp,
      String id, @Nullable String text) {
  ...
  private static int id3tag_set_textinfo_latin1(@Nullable LameGlobalFlags gfp,
      String id, @Nullable String text) {
    long t_mask = FRAME_ID('T', (char) 0, (char) 0, (char) 0);
    int frame_id = toID3v2TagId(id);
    if (frame_id == 0) {
  ...
  public static int id3tag_set_comment(@Nullable LameGlobalFlags gfp,
      @Nullable String lang, @Nullable String desc, String text) {
    if (gfp != null) {
  ...
  public static void id3tag_set_title(LameGlobalFlags gfp,
  ...
  public static void id3tag_set_artist(LameGlobalFlags gfp,
  ...
  public static void id3tag_set_album(LameGlobalFlags gfp,
  ...
  public static void id3tag_set_year(LameGlobalFlags gfp,
  ...
  public static void id3tag_set_comment(LameGlobalFlags gfp,
  ...
  public static int id3tag_set_track(LameGlobalFlags gfp,
  ...
  private static int nextUpperAlpha(String p, int pPos, char x) {
    for (char c = Character.toUpperCase(p.charAt(pPos)); pPos < p
        .length(); c = Character.toUpperCase(p.charAt(pPos++))) {
      if ('A' <= c && c <= 'Z') {
        if (c != x) {
          return pPos;
        }
  ...
  private static boolean sloppyCompared(String p, String q) {
  ...
  @SuppressWarnings("null")
  private static int sloppySearchGenre(String genre) {
    for (int i = 0; i < genre_names.length; ++i) {
  ...
  private static int searchGenre(String genre) {
    for (int i = 0; i < genre_names.length; ++i) {
  ...
  public static int id3tag_set_genre(LameGlobalFlags gfp,
  ...
  private static int set_frame_custom(byte[] frame, int framePos,
      char @Nullable [] fieldvalue) {
    if (fieldvalue != null && fieldvalue[0] != 0) {
      int value = 5;
      int length = new String(fieldvalue, value, fieldvalue.length - value)
          .length();
      ...
      framePos = set_4_byte_value(frame, value,
          (new String(fieldvalue, value, fieldvalue.length - value).length()
              + 1));
  ...
  private static int sizeOfNode(FrameDataNode node) {
    int n = 0;
    n = 10;
    /* header BAG_SIZE */
    n += 1;
  ...
  private static int sizeOfCommentNode(FrameDataNode node) {
    int n = 0;
    n = 10;
    /* header BAG_SIZE */
    n += 1;
  ...
  private static int writeChars(byte[] frame, int framePos, String str,
      int strPos, int n) {
    while (n-- != 0) {
  ...
  private static int writeUcs2s(byte[] frame, int framePos, String str,
      int strPos, int n) {
    while (n-- != 0) {
  ...
  private static int set_frame_comment(byte[] frame, int framePos,
      FrameDataNode node) {
  ...
  private static int set_frame_custom2(byte[] frame, int framePos,
      FrameDataNode node) {
  ...
  private static int set_frame_apic(byte[] frame, int framePos, char[] mimetype,
      byte[] data, int size) {
    /**
     * <PRE>
  ...
  public static int id3tag_set_fieldvalue(LameGlobalFlags gfp,
      String fieldvalue) {
    LameInternalFlags gfc = gfp.internal_flags;
  ...
  /**
   * Based on the settings in the GFP, calculate the tag length,
   * and optionally write the tag to the given buffer.
   * 
   * @param gfp the input settings
   * @param buffer the output buffer to write tag data, or {@code null}
   * if this method was called just to calculate the tag length.
   * 
   * @return the tag length in bytes
   */
  public static int lame_get_id3v2_tag(LameGlobalFlags gfp, byte @Nullable [] buffer) {
    LameInternalFlags gfc = gfp.internal_flags;
    if ((gfc.tag_spec.flags & V1_ONLY_FLAG) != 0)
      return 0;

    /* calculate length of four fields which may not fit in verion 1 tag */
    int title_length = gfc.tag_spec.title != null ? gfc.tag_spec.title.length()
        : 0;
    int artist_length = gfc.tag_spec.artist != null
        ? gfc.tag_spec.artist.length()
        : 0;
    int album_length = gfc.tag_spec.album != null ? gfc.tag_spec.album.length()
        : 0;
    int comment_length = gfc.tag_spec.comment != null
        ? gfc.tag_spec.comment.length()
        : 0;
    ...
    if ((gfc.tag_spec.flags & (ADD_V2_FLAG | V2_ONLY_FLAG)) != 0
      ...
      /* calulate BAG_SIZE of tag starting with 10-byte tag header */
      tag_size = 10;
      ...
      /* clear flags byte */
      buffer[p++] = 0;
      /* calculate and set tag BAG_SIZE = total BAG_SIZE - header BAG_SIZE */
      adjusted_tag_size = tag_size - 10;
      /*
       * encode adjusted BAG_SIZE into four bytes where most significant bit is
       * clear in each byte, for 28-bit total
       */
      ...
      /*
       * NOTE: The remainder of the tag (frames and padding, if any) are not
       * "unsynchronized" to prevent false MPEG audio headers from appearing in
       * the bitstream. Why? Well, most players and utilities know how to skip
       * the ID3 version 2 tag by now even if they don't read its contents, and
       * it's actually very unlikely that such a false "sync" pattern would
       * occur in just the simple text frames added here.
       */
      ...
      for (i = 0; i < gfc.tag_spec.num_values; ++i) {
        p = set_frame_custom(buffer, p,
            gfc.tag_spec.values.get(i).toCharArray());
      }
      if (albumart_mime != null) {
  ...
  public int id3tag_write_v2(LameGlobalFlags gfp) {
    LameInternalFlags gfc = gfp.internal_flags;
    assert gfc != null;
    if ((gfc.tag_spec.flags & CHANGED_FLAG) != 0
      ...
      int n = lame_get_id3v2_tag(gfp, null);
      tag = new byte[n];
      tag_size = lame_get_id3v2_tag(gfp, tag);
      if (tag_size > n) {
        return -1;
      }
  ...
  private static int set_text_field(byte[] field, int fieldPos,
      @Nullable String text, int size, int pad) {
    int textPos = 0;
    while (size-- != 0) {
  ...
  public static int lame_get_id3v1_tag(@Nullable LameGlobalFlags gfp,
      byte @Nullable [] buffer, int size) {
    int tag_size = 128;
    ...
    if ((gfc.tag_spec.flags & CHANGED_FLAG) != 0
      ...
      p = set_text_field(buffer, p, gfc.tag_spec.year != 0 ? year : null, 4,
          pad);
  ...
  public int id3tag_write_v1(LameGlobalFlags gfp) {
    ...
    return n; /* ok, tag has fixed BAG_SIZE of 128 bytes, well below 2GB */
  }
