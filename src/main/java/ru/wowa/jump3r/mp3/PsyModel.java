...
public class PsyModel {
	...
	/* BAG_SIZE of each partition band, in barks: */
	 ...
	 * </PRE>
	 */
	private static float psycho_loudness_approx(float[] energy,
	...
	private static void compute_ffts(LameGlobalFlags gfp,
			float fftenergy[], float fftenergy_s[][],
			float wsamp_l[][], int wsamp_lPos,
			float wsamp_s[][][], int wsamp_sPos, int gr_out,
			int chn, float[] buffer[], int bufPos)	{
		LameInternalFlags gfc = gfp.internal_flags;
		if (chn < 2) {
			FFT.fft_long(wsamp_l[wsamp_lPos], chn, buffer, bufPos);
			FFT.fft_short(wsamp_s[wsamp_sPos], chn, buffer, bufPos);
		}
		/* FFT data for mid and side channel is derived from L & R */
			...
			for (int b = 2; b >= 0; --b) {
				for (int j = Encoder.BLKSIZE_s - 1; j >= 0; --j) {
					float l = wsamp_s[wsamp_sPos + 0][b][j];
					float r = wsamp_s[wsamp_sPos + 1][b][j];
		...
		for (int b = 2; b >= 0; --b) {
			...
			for (int j = Encoder.BLKSIZE_s / 2 - 1; j >= 0; --j) {
				float re = (wsamp_s[wsamp_sPos + 0])[b][Encoder.BLKSIZE_s
						/ 2 - j];
				float im = (wsamp_s[wsamp_sPos + 0])[b][Encoder.BLKSIZE_s
						/ 2 + j];
	 ...
	 */
	private float mask_add(float m1, float m2, int kk, int b,
			LameInternalFlags gfc, int shortblock) {
		float ratio;
	 ...
	 */
	private static void calc_interchannel_masking(LameGlobalFlags gfp,
			float ratio) {
		LameInternalFlags gfc = gfp.internal_flags;
	 ...
	 */
	private static void msfix1(LameInternalFlags gfc) {
		for (int sb = 0; sb < Encoder.SBMAX_l; sb++) {
	 ...
	 * Naoki Shibata 2000
	 */
	private static void ns_msfix(LameInternalFlags gfc, float msfix,
			float athadjust) {
		float msfix2 = msfix;
	 ...
	 */
	private static void convert_partition2scalefac_s(LameInternalFlags gfc,
	 ...
	 */
	private static void convert_partition2scalefac_l(LameInternalFlags gfc,
	...
	private static void compute_masking_s(LameGlobalFlags gfp,
			float fftenergy_s[][], float[] eb, float[] thr,
			int chn, int sblock) {
		LameInternalFlags gfc = gfp.internal_flags;
		int j, b;
	...
	private static void block_type_set(LameGlobalFlags gfp,
			int[] uselongblock, int[] blocktype_d,
			int[] blocktype) {
		LameInternalFlags gfc = gfp.internal_flags;
	...
	private static float NS_INTERP(float x, float y, float r) {
	...
	private static float pecalc_s(III_psy_ratio mr,
			float masking_lower) {
	...
	private static float pecalc_l(III_psy_ratio mr,
			float masking_lower) {
	...
	private static void calc_energy(LameInternalFlags gfc,
			float[] fftenergy, float[] eb, float[] max,
			float[] avg) {
		int b, j;
	...
	private static void calc_mask_index_l(LameInternalFlags gfc,
			float[] max, float[] avg, int[] mask_idx) {
	...
	public int L3psycho_anal_ns(LameGlobalFlags gfp,
			float[] buffer[], int bufPos, int gr_out,
			III_psy_ratio masking_ratio[][],
			III_psy_ratio masking_MS_ratio[][],
			float percep_entropy[], float percep_MS_entropy[],
			float energy[], int blocktype_d[]) {
		/*
	...
	private static void vbrpsy_compute_fft_l(LameGlobalFlags gfp,
			float[] buffer[], int bufPos, int chn,
			int gr_out, float fftenergy[], float wsamp_l[][],
			int wsamp_lPos) {
		LameInternalFlags gfc = gfp.internal_flags;
		if (chn < 2) {
			FFT.fft_long(wsamp_l[wsamp_lPos], chn, buffer, bufPos);
	...
	private static void vbrpsy_compute_fft_s(float[] buffer[],
			int bufPos, int chn, int sblock,
			float fftenergy_s[][], float wsamp_s[][][],
			int wsamp_sPos) {
		if (sblock == 0 && chn < 2) {
			FFT.fft_short(wsamp_s[wsamp_sPos], chn, buffer, bufPos);
		}
	 ...
	 * compute loudness approximation (used for ATH auto-level adjustment)
	 */
	private static void vbrpsy_compute_loudness_approximation_l(LameGlobalFlags gfp,
	 ...
	 * / handling.
	 */
	private static void vbrpsy_attack_detection(LameGlobalFlags gfp,
			float[] buffer[], int bufPos, int gr_out,
			III_psy_ratio masking_ratio[][],
			III_psy_ratio masking_MS_ratio[][],
			float energy[], float sub_short_factor[][],
			int ns_attacks[][], int uselongblock[]) {
		float ns_hpfsmpl[][] = new float[2][576];
		LameInternalFlags gfc = gfp.internal_flags;
		...
		for (int chn = 0; chn < n_chn_psy; chn++) {
			...
			float[] pf = ns_hpfsmpl[chn & 1];
			int pfPos = 0;
			float attackThreshold = (chn == 3) ? gfc.nsPsy.attackthre_s
					: gfc.nsPsy.attackthre;
			int ns_uselongblock = 1;

			if (chn == 2) {
				for (int i = 0, j = 576; j > 0; ++i, --j) {
					float l = ns_hpfsmpl[0][i];
	...
	private static void vbrpsy_skip_masking_s(LameInternalFlags gfc,
			int chn, int sblock) {
		if (sblock == 0) {
	...
	private static void vbrpsy_skip_masking_l(LameInternalFlags gfc,
			int chn) {
	...
	private static void psyvbr_calc_mask_index_s(LameInternalFlags gfc,
			float[] max, float[] avg, int[] mask_idx) {
		int last_tab_entry = tab.length - 1;
		int b = 0;
		float a = avg[b] + avg[b + 1];
	...
	private void vbrpsy_compute_masking_s(LameGlobalFlags gfp,
			float[] fftenergy_s[], float[] eb, float[] thr,
			int chn, int sblock) {
		LameInternalFlags gfc = gfp.internal_flags;
		float max[] = new float[Encoder.CBANDS], avg[] = new float[Encoder.CBANDS];
	...
	private void vbrpsy_compute_masking_l(LameInternalFlags gfc,
			float fftenergy[], float eb_l[], float thr[],
			int chn) {
		float max[] = new float[Encoder.CBANDS], avg[] = new float[Encoder.CBANDS];
	...
	private static void vbrpsy_compute_block_type(LameGlobalFlags gfp,
			int[] uselongblock) {
		LameInternalFlags gfc = gfp.internal_flags;
	...
	private static void vbrpsy_apply_block_type(LameGlobalFlags gfp,
			int[] uselongblock, int[] blocktype_d) {
		LameInternalFlags gfc = gfp.internal_flags;
	 ...
	 * compute M/S thresholds from Johnston & Ferreira 1992 ICASSP paper
	 */
	private static void vbrpsy_compute_MS_thresholds(float eb[][],
			float thr[][], float cb_mld[], float ath_cb[],
			float athadjust, float msfix, int n) {
	...
	public int L3psycho_anal_vbr(LameGlobalFlags gfp,
			float[] buffer[], int bufPos, int gr_out,
			III_psy_ratio masking_ratio[][],
			III_psy_ratio masking_MS_ratio[][],
			float percep_entropy[], float percep_MS_entropy[],
			...
			for (int sblock = 0; sblock < 3; sblock++) {
				for (int chn = 0; chn < n_chn_psy; ++chn) {
					...
					if (uselongblock[ch01] != 0) {
						...
						/* compute masking thresholds for short blocks */
						wsamp_s = wsamp_S;
						vbrpsy_compute_fft_s(buffer, bufPos, chn, sblock, fftenergy_s,
								wsamp_s, ch01);
	...
	private static float s3_func_x(float bark, float hf_slope) {
	...
	private static float norm_s3_func_x(float hf_slope) {
	 ...
	 */
	private static float s3_func(float bark) {
	 ...
	 */
	private static float freq2bark(float freq) {
	...
	private static int init_numline(int[] numlines, int[] bo,
			int[] bm, float[] bval, float[] bval_width,
			float[] mld, float[] bo_w, float sfreq,
			int blksize, int[] scalepos, float deltafreq,
			int sbmax) {
	...
	private static float[] init_s3_values(int s3ind[][], int npart,
			float[] bval, float[] bval_width, float[] norm,
			boolean use_old_s3) {
	...
	private static float stereo_demask(double f) {
		/* setup stereo demasking thresholds */
		/* formula reverse enginerred from plot in paper */
	 ...
	 */
	public int psymodel_init(LameGlobalFlags gfp) {
		LameInternalFlags gfc = gfp.internal_flags;
		int i;
		...
		for (i = 0; i < gfc.npart_l; i++) {
			...
			for (int k = 0; k < gfc.numlines_l[i]; k++, j++) {
				float freq = sfreq * j / (1000.0f * Encoder.BLKSIZE);
				float level;
				/*
		...
		for (i = 0; i < gfc.npart_s; i++) {
			...
			for (int k = 0; k < gfc.numlines_s[i]; k++, j++) {
				float freq = sfreq * j / (1000.0f * Encoder.BLKSIZE_s);
				float level;
				/* freq = Min(.1,freq); *//*
		...
		if (gfp.ATHtype != -1) {
			...
			float freq;
			float freq_inc = (float) gfp.out_samplerate
		...
		j = 0;
		for (i = 0; i < gfc.npart_l; i++) {
			float freq = sfreq * (j + gfc.numlines_l[i] / 2)
		...
		j = 0;
		for (i = 0; i < gfc.npart_s; i++) {
			float freq = sfreq * (j + gfc.numlines_s[i] / 2)
	 ...
	 */
	private static float ATHformula_GB(float f, float value) {
		/**
		 * <PRE>
	...
	public static float ATHformula(float f, LameGlobalFlags gfp) {
		float ath;
		switch (gfp.ATHtype) {
		case 0:
