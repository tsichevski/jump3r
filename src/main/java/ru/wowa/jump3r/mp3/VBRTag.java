...
public class VBRTag {
	...
	public void setModules(Lame lame, BitStream bs, Version v) {
		this.lame = lame;
		this.bs = bs;
		this.v = v;
	}
	 ...
	 *    4 bytes for frame BAG_SIZE
	 *    4 bytes for stream BAG_SIZE
	...
	/**
	 * The BAG_SIZE of the Xing header MPEG-1, bit rate in kbps.
	 */
	private static final int XING_BITRATE1 = 128;
	/**
	 * The BAG_SIZE of the Xing header MPEG-2, bit rate in kbps.
	 */
	private static final int XING_BITRATE2 = 64;
	/**
	 * The BAG_SIZE of the Xing header MPEG-2.5, bit rate in kbps.
	 */
	...
	private static void addVbr(VBRSeekInfo v, int bitrate) {
		v.nVbrNumFrames++;
		v.sum += bitrate;
		v.seen++;
		...
		if (v.pos < VBRSeekInfo.BAG_SIZE) {
			v.bag[v.pos] = v.sum;
			v.pos++;
			v.seen = 0;
		}
		if (v.pos == VBRSeekInfo.BAG_SIZE) {
			for (int i = 1; i < VBRSeekInfo.BAG_SIZE; i += 2) {
				v.bag[i / 2] = v.bag[i];
			}
			v.want *= 2;
			v.pos /= 2;
	...
	private static void xingSeekTable(VBRSeekInfo v, byte[] t) {
		if (v.pos <= 0)
			return;
	 ...
	 *            global flags
	 */
	public static void addVbrFrame(LameGlobalFlags gfp) {
	 ...
	 */
	private static int extractInteger(byte[] buf, int bufPos) {
	 ...
	 */
	private static void createInteger(byte[] buf, int bufPos,
			int value) {
	 ...
	 *            integer value to write
	 */
	private static void createShort(byte[] buf, int bufPos, int value) {
	 ...
	 */
	private static boolean isVbrTag(byte[] buf, int bufPos) {
	...
	private static byte shiftInBitsValue(byte x, int n, int v) {
	 ...
	 *            header
	 */
	private static void setLameTagFrameHeader(LameGlobalFlags gfp,
			byte[] buffer) {
		LameInternalFlags gfc = gfp.internal_flags;
	 ...
	 * @return VBR tag data
	 */
	public static VBRTagData getVbrTag(byte[] buf) {
	 ...
	 *            global flags
	 */
	public void InitVbrTag(LameGlobalFlags gfp) {
		LameInternalFlags gfc = gfp.internal_flags;
		 ...
		 * BAG_SIZE determined by sampling frequency (MPEG1)
	 ...
	 * @return
	 */
	private static int crcUpdateLookup(int value, int crc) {
		int tmp = crc ^ value;
		crc = (crc >> 8) ^ crc16Lookup[tmp & 0xff];
		return crc;
	}

	public static void updateMusicCRC(int[] crc, byte[] buffer,
			int bufferPos, int size) {
	 ...
	 */
	private static int putLameVBR(LameGlobalFlags gfp, int musicLength,
			byte[] streamBuffer, int streamBufferPos, int crc) {
		LameInternalFlags gfc = gfp.internal_flags;
		...
		String version = Version.getLameVeryShortVersion();
		int vbr;
		int revision = 0x00;
		int revMethod;
		...
		if (gfc.findPeakSample)
			peakSignalAmplitude = Math
					.abs((int) (((gfc.PeakSample) / 32767.0)
							* Math.pow(2, 23) + .5));
	...
	private static int skipId3v2(RandomAccessFile fpStream) throws IOException {
		...
		if (!new String(id3v2Header, "ISO-8859-1").startsWith("ID3")) {
			/*
			 * the tag BAG_SIZE (minus the 10-byte header) is encoded into four
	...
	public static int getLameTagFrame(LameGlobalFlags gfp,
			byte[] buffer) {
		LameInternalFlags gfc = gfp.internal_flags;
		...
		// Put total audio stream BAG_SIZE, including Xing/LAME Header
	 ...
	 *             I/O error
	 */
	public static int putVbrTag(LameGlobalFlags gfp,
			RandomAccessFile stream) throws IOException {
		LameInternalFlags gfc = gfp.internal_flags;
		...
		// Get file BAG_SIZE, abort if file has zero length.
		if (stream.length() == 0)
			return -1;
