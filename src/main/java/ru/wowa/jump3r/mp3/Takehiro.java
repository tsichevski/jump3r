...
public class Takehiro {
	...
	public void setModules(QuantizePVT qupvt) {
		this.qupvt = qupvt;
	}
	 ...
	 */
	private static void quantize_lines_xrpow_01(int l, float istep, float[] xr,
			int xrPos, int[] ix, int ixPos) {
		float compareval0 = (1.0f - 0.4054f) / istep;
	 ...
	 */
	private void quantize_lines_xrpow(int l, float istep, float[] xr,
			int xrPos, int[] ix, int ixPos) {
		assert (l > 0);
	 ...
	 */
	private void quantize_xrpow(float[] xp, int[] pi, float istep,
			GrInfo codInfo, CalcNoiseData prevNoise) {
	 ...
	 * ix_max
	 */
	private static int ix_max(int[] ix, int ixPos, int endPos) {
		int max1 = 0, max2 = 0;
	...
	private static int count_bit_ESC(int[] ix, int ixPos, int end, int t1,
			int t2, Bits s) {
		/* ESC-table is used */
	...
	private static int count_bit_noESC(int[] ix, int ixPos, int end, Bits s) {
		/* No ESC-words */
		int sum1 = 0;
	...
	private static int count_bit_noESC_from2(int[] ix, int ixPos, int end,
			int t1, Bits s) {
		/* No ESC-words */
		int sum = 0, sum2;
	...
	private static int count_bit_noESC_from3(int[] ix, int ixPos, int end,
			int t1, Bits s) {
		/* No ESC-words */
		int sum1 = 0;
		int sum2 = 0;
	 ...
	 */
	private static int choose_table(int[] ix, int ixPos, int endPos,
			Bits s) {
	 ...
	 * count_bit
	 */
	public static int noquant_count_bits(LameInternalFlags gfc,
			GrInfo gi, @Nullable CalcNoiseData prev_noise) {
		int[] ix = gi.l3_enc;
		...
		for (; i > 3; i -= 4) {
			...
			if ((((long) ix[i - 1] | (long) ix[i - 2] | ix[i - 3] | ix[i - 4]) & 0xffffffffL) > 1L)
				break;
	...
	public int count_bits(LameInternalFlags gfc, float[] xr,
			GrInfo gi, @Nullable CalcNoiseData prev_noise) {
		int[] ix = gi.l3_enc;

		/* since quantize_xrpow uses table lookup, we need to check this first: */
		float w = (QuantizePVT.IXMAX_VAL) / qupvt.IPOW20(gi.global_gain);
		...
		if ((gfc.substep_shaping & 2) != 0) {
			...
			float roundfac = 0.634521682242439f / qupvt.IPOW20(gain);
	 ...
	 */
	private static void recalc_divide_init(LameInternalFlags gfc,
			GrInfo cod_info, int[] ix, int r01_bits[],
		...
		for (int r0 = 0; r0 < 16; r0++) {
			int a1 = gfc.scalefac_band.l[r0 + 1];
			if (a1 >= bigv)
				break;
			int r0bits = 0;
	...
	private static void recalc_divide_sub(LameInternalFlags gfc,
			GrInfo cod_info2, GrInfo gi, int[] ix,
			int r01_bits[], int r01_div[], int r0_tbl[],
			int r1_tbl[]) {
	...
	public static void best_huffman_divide(LameInternalFlags gfc,
			GrInfo gi) {
	...
	private static void scfsi_calc(int ch, IIISideInfo l3_side) {
		int sfb;
	 ...
	 * not be re-encoded.
	 */
	public void best_scalefac_store(LameInternalFlags gfc, int gr,
			int ch, IIISideInfo l3_side) {
		...
		if (recalc != 0) {
			if (gfc.mode_gr == 2) {
				...
				scale_bitcount_lsf(gi);
			}
		}
	}

	private static boolean all_scalefactors_not_negative(int[] scalefac, int n) {
		for (int i = 0; i < n; ++i) {
			if (scalefac[i] < 0)
				return false;
		}
	 ...
	 */
	public boolean scale_bitcount_lsf(GrInfo cod_info) {
