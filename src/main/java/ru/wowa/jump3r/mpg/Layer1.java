...
public class Layer1 {
	...
	private static void I_step_one(final mpstr_tag mp, final int balloc[],
	...
	private void I_step_two(final mpstr_tag mp, final float fraction[][],
	    ...
	    if (fr.stereo == 2) {
	        ...
	        for (sample = 0, i = 0; i < jsbound; i++) {
	            n = balloc[ba++];
	            if (n!=0)
	            	fraction[0][f0++] = (((-1) << n) + (smpb[sample++]) + 1) * common.muls[n + 1][scale_index[sca++]];
	            else
	            	fraction[0][f0++] = 0.0f;
	            n = balloc[ba++];
	            if (n!=0)
	            	fraction[1][f1++] = (((-1) << n) + (smpb[sample++]) + 1) * common.muls[n + 1][scale_index[sca++]];
	            else
	            	fraction[1][f1++] = 0.0f;
	        }
	        for (i = jsbound; i < MPG123.SBLIMIT; i++) {
