...
package ru.wowa.jump3r.mpg;

import ru.wowa.jump3r.mp3.VBRTag;
import ru.wowa.jump3r.mp3.VBRTagData;
import ru.wowa.jump3r.mpg.Decode.Factory;
import ru.wowa.jump3r.mpg.MPGLib.ProcessedBytes;
import ru.wowa.jump3r.mpg.MPGLib.buf;
import ru.wowa.jump3r.mpg.MPGLib.mpstr_tag;

public class Interface {

  private Common common;

  public Interface(Common c) {
    common = c;
  ...
  static void ExitMP3(mpstr_tag mp) {
    buf b, bn;
  ...
  static buf addbuf(mpstr_tag mp, byte[] buf, int bufPos, int size) {
    buf nbuf = new buf();
  ...
  static void remove_buf(mpstr_tag mp) {
    buf buf = mp.tail;
  ...
  static int read_buf_byte(mpstr_tag mp) {
    int b;
  ...
  static void read_head(mpstr_tag mp) {
    long head;
  ...
  static void copy_mp(mpstr_tag mp, int size, byte[] ptr, int ptrPos) {
    int len = 0;
  ...
  /*
   * traverse mp data structure without changing it (just like sync_buffer) pull
   * out Xing bytes call vbr header check code from LAME if we find a header,
   * parse it and also compute the VBR header BAG_SIZE if no header, do nothing.
   * 
   ...
   * syncword
   */
  static int check_vbr_header(mpstr_tag mp, int bytes) {
    int i, pos;
    buf buf = mp.tail;
    ...
    VBRTagData pTagData = VBRTag.getVbrTag(xing);
    ...
    if (mp.vbr_header) {
      ...
      /*
       * fprintf(
       * stderr,"hip: Xing VBR header dectected.  MP3 file has %i frames\n",
       * pTagData.frames);
       */
  ...
  static int sync_buffer(mpstr_tag mp, boolean free_match) {
    /*
     ...
     * samplerate. return number of bytes in mp, before the header return -1 if
     * header is not found
     */
    int b[] = { 0, 0, 0, 0 };
    int i, pos;
  ...
  interface ISynth {
    <T> int synth_1to1_mono_ptr(mpstr_tag mp, float[] in, int inPos, T[] out,
        ProcessedBytes p, Factory<T> tFactory);

    <T> int synth_1to1_ptr(mpstr_tag mp, float[] in, int inPos, int i, T[] out,
        ProcessedBytes p, Factory<T> tFactory);
  }

  <T> int decodeMP3_clipchoice(mpstr_tag mp, byte[] in, int inPos, int isize, T[] out, ProcessedBytes done, ISynth synth,
      Factory<T> tFactory) {
    ...
    if (!mp.header_parsed) {
      ...
      if (mp.fsizeold == -1 || mp.sync_bitstream != 0) {
        ...
        if (mp.vbr_header) {
          ...
          if (bytes + vbrbytes > mp.bsize) {
            /*
             * fprintf(
             * stderr,"hip: not enough data to parse entire Xing header\n");
             */
            return MPGLib.MP3_NEED_MORE;
          }

          /*
           * read in Xing header. Buffer data in case it is used by a non zero
           * main_data_begin for the next frame, but otherwise dont decode Xing
           * header
           */
          // #ifdef HIP_DEBUG
      ...
      if (bytes > 0) {
        /*
         * there were some extra bytes in front of header. bitstream problem,
         * but we are now resynced should try to buffer previous data in case
         * new frame has nonzero main_data_begin, but we need to make sure we do
         * not overflow buffer
         */
        int size;
        System.err.printf(
        ...
        if (size > MPG123.MAXFRAMESIZE) {
          /*
           * wordpointer buffer is trashed. probably cant recover, but try
           * anyway
           */
          System.err.printf(
              "hip: wordpointer trashed.  BAG_SIZE=%i (%i)  bytes=%i \n", size,
              MPG123.MAXFRAMESIZE, bytes);
          size = 0;
          mp.wordpointer = mp.bsspace[mp.bsnum];
          mp.wordpointerPos = 512;
        }
        ...
        /*
         * buffer contains 'BAG_SIZE' data right now we want to add 'bytes' worth of
         * data, but do not exceed MAXFRAMESIZE, so we through away 'i' bytes
         */
      ...
      if (mp.fsizeold == -1) {
        // #ifdef HIP_DEBUG
        // fprintf(stderr, "hip: not parsing the rest of the data of the first
        // header\n");
        // #endif
        return MPGLib.MP3_NEED_MORE;
      }
    ...
    /*
     * remaining bits are ancillary data, or reservoir for next frame If free
     * format, scan stream looking for next frame to determine mp.framesize
     */
    if (mp.free_format) {
      if (mp.old_free_format) {
        ...
        /*
         * fprintf(
         * stderr,"hip: freeformat bitstream:  estimated bitrate=%ikbs  \n",
  ...
  <T> int decodeMP3(MPGLib.mpstr_tag mp, byte []in, int bufferPos, int isize, T[] out, int osize, ProcessedBytes done,
      Factory<T> tFactory) {
    if (osize < 2304) {
    ...
    ISynth synth = new ISynth() {
      ...
      @Override
      public <X> int synth_1to1_mono_ptr(mpstr_tag mp, float[] in, int inPos,
          X[] out, ProcessedBytes p, Factory<X> tFactory) {
        return decode.synth_1to1_mono(mp, in, inPos, out, p, tFactory);
      }

      @Override
      public <X> int synth_1to1_ptr(mpstr_tag mp, float[] in, int inPos, int i,
          X[] out, ProcessedBytes p, Factory<X> tFactory) {
  ...
  <T> int decodeMP3_unclipped(MPGLib.mpstr_tag mp, byte []in, int bufferPos, int isize, T[] out, int osize,
      ProcessedBytes done, Factory<T> tFactory) {
    /*
    ...
    ISynth synth = new ISynth() {
      ...
      public <X> int synth_1to1_mono_ptr(mpstr_tag mp, float[] in, int inPos,
          X[] out, ProcessedBytes p, Factory<X> tFactory) {
        return decode.synth_1to1_mono_unclipped(mp, in, inPos, out, p,
            tFactory);
      }
