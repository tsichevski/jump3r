...
import ru.wowa.jump3r.mp3.MP3Data;
import ru.wowa.jump3r.mp3.PlottingData;
...
public class MPGLib {
  ...
  public MPGLib(Interface i, Common c) {
    interf = i;
    common = c;
  }
  ...
  public static class mpstr_tag {
    buf head;
    @Nullable buf tail; /* buffer linked list pointers, tail points to oldest buffer */
    ...
    /*
     * header_parsed, side_parsed and data_parsed must be all set 1 before the
     * full frame has been parsed
     */
    boolean header_parsed; /* 1 = header of current frame has been parsed */
    boolean side_parsed; /*
                          * 1 = header of sideinfo of current frame has been
                          * parsed
                          */
    boolean data_parsed;
    ...
    int framesize;
    int ssize; /*
                * number of bytes used for side information, including 2 bytes
                * for CRC-16 if present
                */
    int dsize;
    int fsizeold; /* BAG_SIZE of previous frame, -1 for first */
    int fsizeold_nopadding;
    Frame fr = new Frame(); /* holds the parameters decoded from the header */
    byte bsspace[][] = new byte[2][MPG123.MAXFRAMESIZE
        + 1024]; /* bit stream space used ???? */ /* MAXFRAMESIZE */
  ...
  protected static <DST_TYPE, SRC_TYPE> void COPY_MONO(DST_TYPE[] pcm_l, int pcm_lPos,
  ...
  /* copy stereo samples */
  @SuppressWarnings("unchecked")
    protected static <DST_TYPE, SRC_TYPE> void COPY_STEREO(DST_TYPE[] pcm_l,
        int pcm_lPos, DST_TYPE[] pcm_r, int pcm_rPos, int processed_samples,
        SRC_TYPE[] p) {
    int p_samples = 0;
    for (int i = 0; i < processed_samples; i++) {
      pcm_l[pcm_lPos++] = (DST_TYPE) (p[p_samples++]);
  ...
  interface IDecoder {
    <T> int decode(mpstr_tag mp, byte[] in, int bufferPos, int isize, T[] out,
        int osize, ProcessedBytes done, Factory<T> tFactory);
  }
  ...
  /*
   * For lame_decode: return code -1 error 0 ok, but need more data before
   * outputing any samples n number of samples output. either 576 or 1152
   * depending on MP3 file.
   */
  private static <S, T> int decode1_headersB_clipchoice(mpstr_tag pmp, byte[] buffer,
      int bufferPos, int len, S pcm_l[], int pcm_lPos, S pcm_r[], int pcm_rPos,
      MP3Data mp3data, Enc enc, T[] p, int psize, IDecoder decodeMP3_ptr,
      Factory<T> tFactory) {
    ...
    /*
     * three cases: 1. headers parsed, but data not complete
     * pmp.header_parsed==1 pmp.framesize=0 pmp.fsizeold=BAG_SIZE of last frame, or
     * 0 if this is first frame
     *
     * 2. headers, data parsed, but ancillary data not complete
     * pmp.header_parsed==1 pmp.framesize=BAG_SIZE of frame pmp.fsizeold=BAG_SIZE of
     * last frame, or 0 if this is first frame
     *
     * 3. frame fully decoded: pmp.header_parsed==0 pmp.framesize=0
     * pmp.fsizeold=BAG_SIZE of frame (which is now the last frame)
     *
     */
    if (pmp.header_parsed || pmp.fsizeold > 0 || pmp.framesize > 0) {
      ...
      /*
       * free format, we need the entire frame before we can determine the
       * bitrate. If we haven't gotten the entire frame, bitrate=0
       */
      if (pmp.fsizeold > 0) /*
                             * works for free format and fixed, no overrun,
        ...
        mp3data.bitrate = Common.tabsel_123[pmp.fr.lsf][pmp.fr.lay
            - 1][pmp.fr.bitrate_index];
  ...
  public static int hip_decode_exit(mpstr_tag hip) {
    if (hip != null) {
      Interface.ExitMP3(hip);
      hip = null;
    }
    return 0;
  }
  ...
  /*
   * we forbid input with more than 1152 samples per channel for output in the
   * unclipped mode
   */
  ...
  public int hip_decode1_unclipped(mpstr_tag hip, byte[] buffer, int bufferPos,
    ...
    int decode1_headersB_clipchoice = decode1_headersB_clipchoice(hip, buffer,
        bufferPos, len, pcmL, 0, pcmR, 0, mp3data, enc, out, OUTSIZE_UNCLIPPED,
        dec, tFactory);
    for (int i = 0; i < pcmL.length; i++) {
      pcm_l[i] = pcmL[i];
    }
  ...
  /*
   * For lame_decode: return code -1 error 0 ok, but need more data before
   * outputing any samples n number of samples output. Will be at most one frame
   * of MPEG data.
   */
  public int hip_decode1_headers(mpstr_tag hip, byte[] buffer, int len,
      short pcm_l[], short pcm_r[], MP3Data mp3data) {
    Enc enc = new Enc();
    return hip_decode1_headersB(hip, buffer, len, pcm_l, pcm_r, mp3data, enc);
  }

  public int hip_decode1_headersB(mpstr_tag hip, byte[] buffer, int len,
      final short pcm_l[], final short pcm_r[], MP3Data mp3data, Enc enc) {
    assert hip != null;
    IDecoder dec = new IDecoder() {
    ...
    int decode1_headersB_clipchoice = decode1_headersB_clipchoice(hip, buffer,
        0, len, pcmL, 0, pcmR, 0, mp3data, enc, out, OUTSIZE_CLIPPED, dec,
        tFactory);
    for (int i = 0; i < pcmL.length; i++) {
      pcm_l[i] = pcmL[i];
    }
  ...
  static void hip_set_pinfo(mpstr_tag hip, PlottingData pinfo) {
    if (hip != null) {
      hip.pinfo = pinfo;
    }
  }
